...
- next meeting: 09.02 10:30
- next steps:
  - adding Vivianite,
  - look into the bioturbation problem (discriminate between advective and diffusive flux) and
  - try to run a first dynamic scenario and see how far we can go
...
$\ce{3 Fe^{2+}  + 2 HPO4^{2-} <-> Fe3(PO4)2 + 2 H+}$
...
model_species <- c("ALK", "DIC", "TOT_P", "TOT_N5", "TOT_N3", "TOT_S6", "TOT_S2", "CH4",
                   "O2", "N2", "Mn2", "Fe2", "MnO2", "OM", "FeOH3", "FeOH3_P", "FeCO3",
                   "S0", "FeS", "Viv")
...
split_trans_results <- function(trans, grid) {
...
model <- function (t, state, parms, diff_coeffs, adv_vel, solve_equilibrium, precipitation){
  ...
  with (as.list(parms),{
    ...
    if (precipitation & solve_equilibrium) {
      omega_Viv <- (Fe2^3 * HPO4^2 / H^2) / Ksp_Viv
    } else if (precipitation) {
      omega_Viv <- (Fe2^3 * (TOT_P * f_HPO4)^2 / fixed_H^2) / Ksp_Viv
      #omega_Viv <- (Fe2^0.5 * (TOT_P * f_HPO4) / fixed_H) / sqrt(Ksp_Viv)
    } else {
    ...
    #R15 <- ifelse(omega_Viv > 1, k_precip_Viv * log(omega_Viv^2, 1.5), 0)
    #R16 <- ifelse(omega_Viv < 1, k_diss_Viv * Viv * (1 - omega_Viv)^2, 0)
    ...
    if (precipitation & solve_equilibrium) {
      omega_Viv <- Fe2^3 * HPO4^2 / K_eq_Viv
    } else if (precipitation) {
      omega_Viv <- Fe2^3 * (TOT_P * f_HPO4)^2 / K_eq_Viv
    } else {
      omega_viv <- rep(1, N_grid)
    }

    R15 <- ifelse(omega_Viv > 1, k_precip_Viv * (omega_Viv - 1), 0)
    R16 <- ifelse(omega_Viv < 1, k_diss_Viv * Viv * (1 - omega_Viv), 0)
...
if (run_std0) {
  ...
  saveRDS(std0, "last_results/std0.rds")
...
equilibrium_std1 <- solve_tableau(component_total, tableau, logK, N_grid)
...
timesteps <- seq(from = 0, to = 0.1, by = 1/12)
...
if (run_trans) {
  ...
  saveRDS(trans, "last_results/trans.rds")
...
trans_split <- split_trans_results(trans, grid)
...
plot_trans_conc_contour(trans_split, "FeOH3")

plot_trans_conc_contour(trans_split, "Viv")

plot_trans_conc_lines(trans_split, "FeOH3", c(0/12, 3/12, 6/12, 9/12, 1))
...
elemental_composition <- list(
  ...
  P = data.frame(
    species = c("TOT_P", "OM", "FeOH3_P", "Viv"),
    stoic   = c(1, reaction_parameter[["OM_c"]], reaction_parameter[["lambda"]], 2)
  ),
  S = data.frame(
