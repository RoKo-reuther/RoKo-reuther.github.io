...
grid     <- setup.grid.1D(L = length, N = N_grid, dx.1 = 5e-5)
```
...
get_OMdeg_total <- function(grid, grid.svf) {
...
OMdeg_total <- get_OMdeg_total(grid, grid.svf)
```
...
model <- function (t, state, parms, diff_coeffs, adv_vel, k_iri, solve_equilibrium, precipitation){
  ...
  with (as.list(parms),{
    ...
    R7 <- ifelse(omega_siderite > 1, k_precip_siderite * (omega_siderite - 1)^1.2, 0)
    R8 <- ifelse(omega_siderite < 1, k_diss_siderite * FeCO3 * (1 - omega_siderite)^1.2, 0)
    ...
    R13 <- ifelse(omega_FeS > 1, k_precip_FeS * (omega_FeS - 1)^1.2, 0)
    R14 <- ifelse(omega_FeS < 1, k_diss_FeS * FeS * (1 - omega_FeS)^1.2, 0)
    ...
    dCdt.TOT_N3  <- R.TOT_N3 + tran.TOT_N3$dC + I.TOT_N3
