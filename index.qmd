---
title: "Fe Treatment Model"
---


```{r document switches}
# load std from disk (false) or calculate new (true)
run_std <- TRUE
#run_std <- FALSE

# load std0 from disk (false) or calculate new (true)
run_std0 <- TRUE
#run_std0 <- FALSE

# load std1 from disk (false) or calculate new (true)
run_std1 <- TRUE
#run_std1 <- FALSE

# load std2 from disk (false) or calculate new (true)
run_std2 <- TRUE
#run_std2 <- FALSE

# load trans from disk (false) or calculate new (true)
run_trans <- TRUE
run_trans <- FALSE
```

```{r load libraries}
#| message: false

require(readODS)
require(marelac)
require(ReacTran)
require(microbenchmark)
require(plotly)
```

## Quick Info

- next meeting: 21.02.2024 | 09:30
- next steps:
  - add Ammonium oxidation + MnCO3 precip / diss
  - setup version comparable to Wang & Van Cappellen
  - import Terra Nova porewater-data for comparision

## Concept

### Reactions

Reactions affect solute and solid model species differently.

The solid model species are the same occurring in the chemical reaction equations.
Therefore they are affected directly according to the reaction stoichiometry.

Solute model species are either component-totals (e.g. DIC, TOT_P, ...) to be speciated later or species on their own (e.g. O2, CH4).
The effect of reactions on solute species is first formulated for the species occurring in the chemical reaction equations according to the reaction stoichiometry.
In a second step this changes are translated to changes of the component-totals / model species using a translation table.

### Transport

For solutes the (probably) dominant species of a component-total governs the diffusion coefficient (cf code-block "Diffusion Coefficients").
The transport of solids is not affected by species properties.


## Reactions Describing OM Decomposition

### OM Composition

$\ce{(CH2O)_a (NH3)_b (H3PO4)_c}$

- $a = 1$
- $b = 16/106$
- $c = 1/106$


### Rate Constants

- $k_{\alpha} = 0.05 - 1.62 \: yr^{-1}$    [@moodley2005; @tromp1995global]
- $k_{\beta} = 0.025 - 0.0086 \: yr^{-1}$  [@tromp1995global]


### Limitation (L) and Inihibition (I) Terms

#### O2

$L_{\ce{O2}} = \frac{[\ce{O2}]}{K_{\ce{O2}} + [\ce{O2}]}$

$I_{\ce{O2}} = \frac{K_{\ce{O2}}}{K_{\ce{O2}} + [\ce{O2}]}$

with $K_{O2} = 0.001 - 0.03 \: mol \: m^{-3}_{pw}$ [@wang1996multicomponent]

#### NO3-

$L_{\ce{NO3^-}} = \frac{[\ce{NO3^-}]}{K_{\ce{NO3-}} + [\ce{NO3^-}]}$

$I_{\ce{NO3^-}} = \frac{K_{\ce{NO3^-}}}{K_{\ce{NO3^-}} + [\ce{NO3^-}]}$

with $K_{NO3-} = 0.004 - 0.08  \: mol \: m^{-3}_{pw}$ [@wang1996multicomponent]

#### MnO2

$L_{\ce{MnO2^{\alpha}}} = \frac{[\ce{MnO2^{\alpha}}]}{K_{\ce{MnO2}} + [\ce{MnO2^{\alpha}}]}$
 
$I_{\ce{MnO2^{\alpha}}} = \frac{K_{\ce{MnO2}}}{K_{\ce{MnO2}} + [\ce{MnO2^{\alpha}}]}$

with $K_{\ce{MnO2^{\alpha}}} = \{0.004 - 0.032\} \: mol \: {kg}^{-1} \cdot \rho_{dry}$  [@wang1996multicomponent]

#### Fe(OH)3

$L_{\ce{Fe(OH)3_{; tot}^\alpha}} = \frac{[\ce{Fe(OH)3_{; tot}^\alpha}]}{K_{\ce{Fe(OH)3}} + [\ce{Fe(OH)3_{; tot}^\alpha}]}$

$I_{\ce{Fe(OH)3^\alpha}} = \frac{K_{\ce{Fe(OH)3}}}{K_{\ce{Fe(OH)3}} + [\ce{Fe(OH)3_{; tot}^\alpha}]}$

with $K_{\ce{FeOH3}} = \{0.065 - 0.1\} \: mol \: {kg}^{-1} \cdot \rho_{dry}$ [@wang1996multicomponent]

#### SO42-

$L_{\ce{SO4^{2-}}} = \frac{[\ce{SO4^{2-}}]}{K_{\ce{SO4^{2-}}} + [\ce{SO4^{2-}}]}$

$I_{\ce{SO4^{2-}}} = \frac{K_{\ce{SO4^{2-}}}}{K_{\ce{SO4^{2-}}} + [\ce{SO4^{2-}}]}$

with $K_{SO42-} = 0.0016  \: mol \: {m^{-3}_{pw}}$ [@wang1996multicomponent]

#### H3PO4

$L_{\ce{H3PO4}} = \frac{[\ce{H3PO4}]}{K_{\ce{H3PO4}} + [\ce{H3PO4}]}$

with $K_{\ce{H3PO4}} = 10^{-5} \: mol \: m^{-3}_{pw}$ (Reinier)

> formulated for lump sum

### FeOH3 Fractions

$\ce{Fe(OH)3_{; tot}^\alpha} = \ce{Fe(OH)3^{\alpha}} + \ce{Fe(OH)3^\alpha(H3PO4)_{\lambda}}$ with $\lambda = 0.6$

$\chi = \frac{[\ce{Fe(OH)3^{\alpha}}]}{[\ce{Fe(OH)3_{; tot}^\alpha}]}$


### Reactions

#### OM + O2 (R1)

$\ce{OM^{\alpha, \beta} + O2 -> CO2 + b NH3 + c H3PO4 + H2O}$

$R = k_{\alpha, \beta} \: OM^{\alpha, \beta} \: L_{\ce{O2}}$


#### OM + HNO3 (R2)

$\ce{OM^{\alpha, \beta} + 4/5 HNO3 -> CO2 + b NH3 + c H3PO4 + 2/5 N2 + 7/5 H2O}$

$R = k_{\alpha, \beta} \: OM^{\alpha, \beta} \: L_{\ce{NO3-}} \: I_{\ce{O2}}$


#### OM + MnO2 (R3)

$\ce{OM^{\alpha, \beta} + 2 MnO2 + H2O -> CO2 + b NH3 + c H3PO4 + 2 Mn(OH)2}$

$R = k_{\alpha, \beta} \: OM^{\alpha, \beta} \: L_{\ce{MnO2}^\alpha} \: I_{\ce{NO3^-}} \: I_{\ce{O2}}$


#### OM + FeOH3 / FeOH3~H3PO4 (R4)

$\ce{OM^{\alpha, \beta} + 4 (\chi Fe(OH)3^{\alpha} + (1 - \chi) Fe(OH)3^\alpha(H3PO4)_{\lambda} ) -> CO2 + b NH3 + (c + (1- \chi) 4 \lambda) H3PO4 + 4 Fe(OH)2 + 3 H2O}$

$R = k_{\alpha, \beta} \: OM^{\alpha, \beta} \: L_{\ce{Fe(OH)3_{; tot}^\alpha}} \: I_{\ce{MnO2^{\alpha}}} \: I_{\ce{NO3^-}} \: I_{\ce{O2}}$


#### OM + H2SO4 (R5)

$\ce{OM^{\alpha, \beta} + 1/2 H2SO4 -> CO2 + b NH3 + c H3PO4 + 1/2 H2S + H2O}$

$R = k_{\alpha, \beta} \: OM^{\alpha, \beta} \: L_{\ce{SO4^{2-}}} \: I_{\ce{Fe(OH)3^\alpha}} \: I_{\ce{MnO2^{\alpha}}} \: I_{\ce{NO3^-}} \: I_{\ce{O2}}$


#### Methanogenesis (R6)

$\ce{OM^{\alpha, \beta} -> 1/2 CO2 + b NH3 + c H3PO4 + 1/2 CH4}$

$R = k_{\alpha, \beta} \: OM^{\alpha, \beta} \: I_{\ce{SO4^{2-}}} \: I_{\ce{Fe(OH)3^\alpha}} \: I_{\ce{MnO2^{\alpha}}} \: I_{\ce{NO3^-}} \: I_{\ce{O2}}$

## Secondary Redox Reactions

### H2S Oxidation (R11)

$\ce{2 O2 + H2S -> H2SO4}$

$R = k \: [\ce{O2}] \: [\ce{\sum H2S}]$

with $k \geq 1.6 \times 10^2 \: m^3 \: mol^{-1} \: yr^{-1}$ [@wang1996multicomponent]

### SO4-Reduction coupled to AOM (R12)

$\ce{H2SO4 + CH4 -> CO2 + H2S + 2 H2O}$

$R = k \: [\ce{SO4^{2-}}] \: [\ce{CH4}]$

with $k = 10 - 120  \: m^3 \: mol^{-1} \: yr^{-1}$ [@wang1996multicomponent; @rooze2016iron]

## Redox Precipitation / Dissolution Reactions

### FeOH3 / FeOH3~H3PO4-Reduction coupled to Sulphide Oxidation (R9)

$\ce{2 ( \chi Fe(OH)3^{\alpha} + (1-\chi) Fe(OH)3^{\alpha}(H3PO4)_{\lambda} ) + H2S -> 2 Fe(OH)_2 + S^0 + 2 H2O + 2 (1 - \chi) \lambda H3PO4}$

$R = k \: [\ce{Fe(OH)3_{; tot}^{\alpha}}] \: [\ce{\sum H2S}] \quad (mol \: {m_{sf}}^{-3} \: yr^{-3})$

with $k \leq 10^2 \: {m_{pw}}^3 \: mol^{-1} \: yr^{-1}$ [@wang1996multicomponent]

> unit correct???

### FeOH3 / FeOH3~H3PO4-Formation (R10)

$\ce{O2 + 4 Fe(OH)_2 + 2 H2O + 4 L_{\ce{H3PO4}} \lambda H3PO4 -> 4 ( (1 - L_{\ce{H3PO4}}) Fe(OH)3^{\alpha} + L_{\ce{H3PO4}} Fe(OH)3^{\alpha}(H3PO3)_{\lambda} )}$

$R = k \: [\ce{O2}] \: [\ce{Fe^{2+}}]$
 
with $k = 1.4 \times 10^5 \: {m_{pw}}^3 \: mol^{-1} \: yr^{-1}$ [@wang1996multicomponent]


## Precipitation Reactions

### Siderite Precipitation (R7) / Dissolution (R8)

In general:

$\ce{FeCO3 <-> Fe^{2+} + CO3^{2-}}$

In our model:

$\ce{FeCO3 + 2 H2O <-> Fe(OH)2 + H2CO3}$

Precipitation and Dissolution are split in two reactions from which only one is active at a time, depending on the saturation state $\Omega$.

$\Omega = \frac{\ce{[Fe^{2+}]} \cdot \ce{[CO3^{2-}]}}{K_{sp; siderite}} \quad$
with $\quad K_{sp; siderite} = 10^{-10.4} \cdot 1000^2 \; mol^2 \; m^{-6}$

If $\: \Omega > 1$:

- $R_{precipitation} = k_{precip} \cdot (\Omega - 1)^2 \quad$
with $\quad k_{precip} \approx 1.8 \times 10^{2} \; mol \; {m_{sf}}^{-3} \; yr^{-1}$
- $R_{dissolution} = 0$

If $\: \Omega < 1$:

- $R_{precipitation} = 0$
- $R_{dissolution} = - k_{diss} \cdot \ce{[FeCO3]} \cdot (\Omega - 1)^2 \quad$
with $\quad k_{diss} \approx 2.5 \times 10^{-1} \; yr^{-1}$

### FeS Precipitation (R13) / Dissolution (R14)

Chemically:

$\ce{Fe^{2+}  + HS- <-> FeS + H+}$

In our model:

$\ce{Fe(OH)_2 + H2S <-> FeS + 2 H2O}$

Precipitation and Dissolution are split in two reactions from which only one is active at a time, depending on the saturation state $\Omega$.

$\Omega = \frac{ \ce{[Fe^{2+}]} \cdot \ce{[HS^{-}]} }{ \ce{[H+]} \cdot K_{sp; FeS} } \quad$
with $\quad K_{sp; FeS} = 10^{-3.915} \cdot 1000 \; mol \; m^{-3}$

> **phreeqc.dat**  
> FeS(ppt) | FeS + H+ = Fe+2 + HS- | log_k	-3.915  

If $\: \Omega > 1$:

- $R_{precipitation} = k_{precip} \cdot (\Omega - 1)^2 \quad$
with $\quad k_{precip} \approx 2.5 \times 10^{-3} \; mol \; {m_{sf}}^{-3} \; yr^{-1}$ [@markelov2019coupling]
- $R_{dissolution} = 0$

If $\: \Omega < 1$:

- $R_{precipitation} = 0$
- $R_{dissolution} = - k_{diss} \cdot \ce{[FeS]} \cdot (\Omega - 1)^2 \quad$
with $\quad k_{diss} \approx 1 \times 10^{-3} \; yr^{-1}$ [@markelov2019coupling]

### Vivianite Precipitation (R15) / Dissolution (R16)

Chemically:

$\ce{3 Fe^{2+}  + 2 PO4^{2-} <-> Fe3(PO4)2}$

In our model:

$\ce{3 Fe(OH)2 + 2 H3PO4 <-> Fe3(PO4)2 + 6 H2O}$

Precipitation and Dissolution are split in two reactions from which only one is active at a time, depending on the saturation state $\Omega$.

$\Omega = \frac{ \ce{[Fe^{2+}]}^3 \cdot \ce{[HPO_4^{2-}]}^2 }{ \ce{[H+]}^2 \cdot K_{sp; Viv} } \quad$
with $\quad K_{sp; Viv} = 10^{-36} \cdot 1000^3 \; mol^3 \; m^{-9}$

> **phreeqc.dat**  
> Vivianite | Fe3(PO4)2:8H2O = 3 Fe+2 + 2 PO4-3 + 8 H2O | log_k  -36.000

If $\: \Omega > 1$:

- $R_{precipitation} = k_{precip} \cdot (\Omega - 1)^2 \quad$
with $\quad k_{precip} \approx 1.1 \times 10^{-2} \; mol \; {m_{sf}}^{-3} \; yr^{-1}$ [@markelov2019coupling]
- $R_{dissolution} = 0$

If $\: \Omega < 1$:

- $R_{precipitation} = 0$
- $R_{dissolution} = - k_{diss} \cdot \ce{[Fe3(PO4)2]} \cdot (\Omega - 1)^2 \quad$
with $\quad k_{diss} \approx 5.3 \times 10^{-3} \; yr^{-1}$ [@markelov2019coupling]

### CaCO3 Precipitation (R17) / Dissolution (R18)

Chemically:

$\ce{Ca^{2+}  + CO3^{2-} <-> CaCO3}$

In our model:

$\ce{Ca(OH)2 + H2CO3 <-> CaCO3 + 2 H2O}$

Precipitation and Dissolution are split in two reactions from which only one is active at a time, depending on the saturation state $\Omega$.

$\Omega = \frac{ \ce{[Ca^{2+}]} \cdot \ce{[CO3^{2-}]} }{ K_{sp; \ce{CaCO3}} } \quad$
with $\quad K_{sp; \ce{CaCO3}} = 10^{-8.48} \cdot 1000^2 \; mol^2 \; m^{-6}$

> **phreeqc.dat**  
> Calcite | CaCO3 = CO3-2 + Ca+2 | log_k  -8.480

If $\: \Omega > 1$:

- $R_{precipitation} = k_{precip} \cdot (\Omega - 1)^2 \quad$
with $\quad k_{precip} \approx 1 \times 10^{-1} \; mol \; {m_{sf}}^{-3} \; yr^{-1}$ [@markelov2019coupling]
- $R_{dissolution} = 0$

If $\: \Omega < 1$:

- $R_{precipitation} = 0$
- $R_{dissolution} = - k_{diss} \cdot \ce{[CaCO3]} \cdot (\Omega - 1)^2 \quad$
with $\quad k_{diss} \approx 1.25 \times 10^{-1} \; yr^{-1}$ [@markelov2019coupling]

### Apatite Precipitation (R19) / Dissolution (R20)

Chemically:

$\ce{3 Ca^{2+}  + 2 HPO4^{2-} <-> Ca3(PO4)2 + 2 H+}$

In our model:

$\ce{3 Ca(OH)2 + 2 H3PO4 <-> Ca3(PO4)2 + 6 H2O}$

Precipitation and Dissolution are split in two reactions from which only one is active at a time, depending on the saturation state $\Omega$.

$\Omega = \frac{ \ce{[Ca^{2+}]}^3 \cdot \ce{[HPO_4^{2-}]}^2 }{ \ce{[H+]}^2 \cdot K_{sp; Apatite} } \quad$
with $\quad K_{sp; Apatite} = 6 \times 10^{-20} \; mol^3 \; m^{-9}$

> ? Calciumcarbonate or Hydroxyapatite ?

If $\: \Omega > 1$:

- $R_{precipitation} = k_{precip} \cdot (\Omega - 1)^2 \quad$
with $\quad k_{precip} \approx 1.4 \times 10^{-6} \; mol \; {m_{sf}}^{-3} \; yr^{-1}$ [@markelov2019coupling]
- $R_{dissolution} = 0$

If $\: \Omega < 1$:

- $R_{precipitation} = 0$
- $R_{dissolution} = - k_{diss} \cdot \ce{[Apatite]} \cdot (\Omega - 1)^2 \quad$
with $\quad k_{diss} \approx 3.7 \times 10^{-2} \; yr^{-1}$ [@markelov2019coupling]


## Equilibrium Reactions described via Tableau

```{r read tableau}
excel <- read_fods("tableau.fods", sheet = "morel_tableau")

tableau_print <- as.data.frame(excel)

excel[is.na(excel)] <- 0

column_H2O  <- 2
column_logK <- 10
column_species_names <- 1
row_H2O     <- 1

tableau <- as.matrix(excel[-row_H2O,-c(column_H2O, column_logK, column_species_names)])

logK <- as.vector(unlist(excel[-row_H2O, column_logK]))

tableau_species <- unlist(excel[,column_species_names])[-1]
names(tableau_species) <- NULL

model_species <- c("ALK", "DIC", "TOT_P", "TOT_N5", "TOT_N3", "TOT_S6", "TOT_S2", "CH4",
                   "O2", "N2", "Mn2", "Fe2", "Ca2", "MnO2", "OM", "OM_beta", "FeOH3", "FeOH3_P", "FeCO3",
                   "S0", "FeS", "Viv", "CaCO3", "Apatite", "MnCO3")

#excel <- read_xlsx("tableau.xlsx", sheet = "translation_table")
#excel[is.na(excel)] <- 0

rm(column_H2O, column_logK, column_species_names, row_H2O, excel)
```

The Tableau is used to calculate equilibrium speciation of solute species after model run.
Fast equilibrium reactions are neglected during model runtime (so far).

```{r print tableau}
options(knitr.kable.NA = '')
knitr::kable(tableau_print)
```


## Functions

### Save and upload Results

```{r def save_and_upload_results}
#| echo: true
#| code-fold: true

save_and_upload_results <- function(result, result_name, result_type, grid) {

  # result_type: "std" | "trans"

  # save as .rds
  saveRDS(result, paste0("last_results/", result_name, ".rds"))

  # save results as .csv
  ## std
  if (result_type == "std") {
    # species concentrations
    write.csv(result$y, paste0("../temp_drop/", result_name, "_species.csv"))
    # transport
    write.csv(result$transport, paste0("../temp_drop/", result_name, "_transport.csv"))
    # sumR
    write.csv(result$sumR, paste0("../temp_drop/", result_name, "_sumR.csv"))
    # reaction rates
    write.csv(result$rates, paste0("../temp_drop/", result_name, "_rates.csv"))
    # omega
    write.csv(result$omega, paste0("../temp_drop/", result_name, "_omega.csv"))
    # speciation
    try(write.csv(result$solute_equilibrium$species_conc, paste0("../temp_drop/", result_name, "_speciation.csv")), silent = TRUE)
    
  } else if (result_type == "trans") {

  }

  # upload to github repo: git@github.com:RoKo-reuther/temp_drop.git
  system("cd ../temp_drop && git add . && git commit -m 'update' && git push")
}
```

### Solve Tableau

```{r def solve_tableau}
#| echo: true
#| code-fold: true

solve_tableau <- function(component_total, tableau, logK, N_grid, tableau_species) {
  
  # load shared object
  dyn.load("solve_tableau.so")

  res <- .Fortran("solve_tableau",
                  component_total = as.double(component_total),
                  tableau         = matrix(as.double(tableau), ncol = ncol(tableau)),
                  logK            = as.double(logK),
                  N_components    = as.integer(ncol(tableau)),
                  N_species       = as.integer(nrow(tableau)),
                  N_grid          = as.integer(N_grid),
                  iter_pcfm       = rep(as.integer(0), N_grid),
                  iter_newton     = rep(as.integer(0), N_grid),
                  info_newton     = rep(as.integer(0), N_grid),
                  difference      = matrix(as.double(0), ncol = ncol(tableau), nrow = N_grid),
                  species_conc    = matrix(as.double(0), ncol = nrow(tableau), nrow = N_grid),
                  success         = rep(as.integer(0), N_grid)
                  )

  colnames(res$species_conc) <- tableau_species

  return(res)
  
}
```

### Calculate Speciation-Ratios for Fixed pH

```{r def speciation_ratios}
#| echo: true
#| code-fold: true

speciation_ratios <- function(pH, tableau, tableau_species, logK) {

  # determine number of total components TOT_X in tableau
  N_components <- ncol(tableau)

  # prepare vector with "component-concentrations"
  # here we are only interested in ratios at a fixed pH ...
  # so the only one that matters is the first one: TOT_H; the rest is set to 1
  component_conc <- c(log10(10^-pH * 10^3), rep(1, N_components - 1))

  # calculate species concentrations
  species_conc <- as.list(10**(tableau %*% component_conc + logK))

  # name entries
  names(species_conc) <- tableau_species

  # calculate ratios of interest
  with(species_conc, {
    ratios <- c(
      fixed_H = 10^-pH * 10^3,
      f_H2CO3 = H2CO3 / sum(H2CO3, HCO3, CO3),
      f_HCO3  =  HCO3 / sum(H2CO3, HCO3, CO3),
      f_CO3   =   CO3 / sum(H2CO3, HCO3, CO3),
      f_H2S   =   H2S / sum(H2S, HS, S2),
      f_HS    =    HS / sum(H2S, HS, S2),
      f_S2    =    S2 / sum(H2S, HS, S2),
      f_H3PO4 = H3PO4 / sum(H3PO4, H2PO4, HPO4, PO4),
      f_H2PO4 = H2PO4 / sum(H3PO4, H2PO4, HPO4, PO4),
      f_HPO4  = HPO4  / sum(H3PO4, H2PO4, HPO4, PO4),
      f_PO4   = PO4   / sum(H3PO4, H2PO4, HPO4, PO4),
      f_NH4   = NH4   / sum(NH4, NH3),
      f_NH3   = NH3   / sum(NH4, NH3)
    )

    return(ratios)
  })
}
```

### Plot steady.1D Results

```{r def plot functions 1}
#| echo: true
#| code-fold: true

plot_simple_comparision <- function(a, b, a_name, b_name, grid, xlab, ylab, log = "") {
  plot(
    grid$x.mid ~ a,
    ylim = c(max(grid$x.int), min(grid$x.int)),
    xlim = c(min(a, b), max(a, b)),
    type = "l",
    ylab = ylab,
    xlab = xlab,
    lwd = 2,
    col = "gray45",
    lty = 2,
    log = log
  )
  points(
    grid$x.mid ~ b,
    type = "l",
    lwd = 2,
    col = "black"
  )
  legend(
    "bottomright",
    legend = c(a_name, b_name),
    col = c("gray45", "black"),
    lwd = 2,
    lty = c(2, 1),
    box.lwd = 0
  )
}

plot_std_profiles <- function(std_gray, std_black, grid) {

  std_black <- cbind(grid = grid$x.mid, std_black$y)
  
  plot(std_gray, xyswap = TRUE, xlab = "mol/m3_phase", ylab = "depth (m)",
       grid = grid$x.mid,
       which = model_species,
       col = "gray45",
       lty = 2,
       lwd = 2,
       cex = 1.5,
       mfrow = c(1,1),
       obs = std_black,
       obspar = c(type = "l", lwd = 2, col = "black")
  )
}

plot_std_rates <- function(std, reactions, title = "") {
  
  # rcols <- viridis::viridis(length(std$rates))
  # 
  # matplot(y = grid$x.mid,
  #         x = as.data.frame(std$rates),
  #         type = "l", lwd=2, lty=1,
  #         ylim = c(length,0), 
  #         col = rcols,
  #         ylab="depth (m)", 
  #         xlab="reaction rate (mol/(m3_bulk yr))")
  # 
  # legend(
  #   x = "bottomright",
  #   legend = names(std$rates),
  #   lty = 1,
  #   col = rcols <- viridis::viridis(length(std$rates)),
  #   lwd = 2
  # )
  
  data <- as.data.frame(std$rates)
  data$depth <- grid$x.mid
  
  fig <- plot_ly(data, type = 'scatter', mode = 'lines', line = list(width = 3))
  for (rate in reactions) {
    fig <- fig %>% add_trace(y = ~depth, x = std$rates[[rate]], name = rate)
  }
  fig <- fig %>% layout(
    title = title,
    xaxis = list(title = "reaction rate (mol/(m3_bulk yr))"),
    yaxis = list(title = "depth (m)", range = c(length,0)),
    font = list(size = 16)
  )
    
  fig
}


plot_std_rate_comparision <- function(species, std, grid) {

  data <- data.frame(
    d = grid$x.mid,
    t = std$transport[[paste0("tran.", species)]]$dC * 0.8,
    r = std$sumR[[paste0("R.", species)]],
    i = std$irrigation[[paste0("I.", species)]]
  )

  fig <- plot_ly(data, y = ~d) 
  fig <- fig %>% add_trace(x = ~t, name = 'transport',mode = 'lines', type = 'scatter')
  fig <- fig %>% add_trace(x = ~r, name = 'sumR',mode = 'lines', type = 'scatter')
  fig <- fig %>% add_trace(x = ~i, name = 'irrigation',mode = 'lines', type = 'scatter')

  fig <- fig %>% layout(
    title = paste(species),
    xaxis = list(title = "rate (mol/(m3_bulk yr))"),
    yaxis = list(title = "depth (m)", range = c(max(grid$x.int),0)),
    font = list(size = 14)
  )

  return(fig)
}
```

### Plot Runge-Kutta Results

```{r def plot functions 2}
#| echo: true
#| code-fold: true

plot_std_conc_evolution <- function(std_matrix_list, selection, N_lines, grid) {
  
  data <- std_matrix_list[[selection]]
  
  # number of available timesteps
  available_timesteps <- ncol(data)
  
  # selection of N_lines timesteps
  selected_timesteps <- floor(seq.int(1, available_timesteps, length.out = N_lines))
  
  # reduce data
  data <- data[,selected_timesteps]
  
  # plot
  matplot(y = grid$x.mid,
          x = data,
          type = "l",
          ylim = c(max(grid$x.mid), min(grid$x.mid)),
          xlab = "concentration (mol/m^3_phase)",
          ylab = "depth (m)",
          main = selection)
}

plot_std_rate_evolution <- function(std, selection, N_lines) {
  
  data <- std$rates_split[[selection]]
  
  # number of available depths
  available_depths <- nrow(data)
  
  # selection of N_lines depths
  selected_depths <- floor(seq.int(1, available_depths, length.out = N_lines))
  
  # reduce data
  data <- data[selected_depths,]
  
  # plot
  matplot(x = std$times,
          y = t(data),
          type = "l",
          ylab = "rate (mol/(m^3_phase * yr))",
          xlab = "time (a)",
          main = selection)
}
```

### Plot Speciation

```{r def plot_speciation}
#| echo: true
#| code-fold: true

plot_speciation <- function(speciation, grid) {

  rcols <- viridis::viridis(6)

  matplot(y=grid$x.mid, x = speciation[,3:5],
    type="l", lwd=2, lty=1,
    ylim=c(length,0),
    col=c(rcols[3], rcols[6], rcols[1]),
    ylab="depth (m)", 
    xlab="mol/m3_phase"
  )
  legend(
    x = "bottom",
    legend = c("H2CO3", "HCO3", "CO3"),
    lty = 1,
    col = c(rcols[3], rcols[6], rcols[1]),
    lwd = 2
  )

  matplot(y=grid$x.mid, x = speciation[,6:9],
    type="l", lwd=2, lty=1,
    ylim=c(length,0), 
    col=c(rcols[3], rcols[6], rcols[1], rcols[4]),
    ylab="depth (m)", 
    xlab="mol/m3_phase"
  )
  legend(
    x = "bottom",
    legend = c("H3PO4", "H2PO4-", "HPO42-", "PO43-"),
    lty = 1,
    col = c(rcols[3], rcols[6], rcols[1], rcols[4]),
    lwd = 2
  )

  matplot(y=grid$x.mid, x = speciation[,10:11],
    type="l", lwd=2, lty=1,
    ylim=c(length,0), 
    col=c(rcols[3], rcols[6]),
    ylab="depth (m)", 
    xlab="mol/m3_phase"
  )
  legend(
    x = "bottom",
    legend = c("HNO3", "NO3-"),
    lty = 1,
    col = c(rcols[3], rcols[6]),
    lwd = 2
  )

  matplot(y=grid$x.mid, x = speciation[,12:13],
    type="l", lwd=2, lty=1,
    ylim=c(length,0), 
    col=c(rcols[3], rcols[6]),
    ylab="depth (m)", 
    xlab="mol/m3_phase"
  )
  legend(
    x = "bottom",
    legend = c("NH4+", "NH3"),
    lty = 1,
    col = c(rcols[3], rcols[6]),
    lwd = 2
  )

  matplot(y=grid$x.mid, x = speciation[,14:16],
    type="l", lwd=2, lty=1,
    ylim=c(length,0), 
    col=c(rcols[3], rcols[6], rcols[1]),
    ylab="depth (m)", 
    xlab="mol/m3_phase"
  )
  legend(
    x = "bottom",
    legend = c("H2SO4", "HSO4-", "SO42-"),
    lty = 1,
    col = c(rcols[3], rcols[6], rcols[1]),
    lwd = 2
  )

  matplot(y=grid$x.mid, x = speciation[,17:19],
    type="l", lwd=2, lty=1,
    ylim=c(length,0), 
    col=c(rcols[3], rcols[6], rcols[1]),
    ylab="depth (m)", 
    xlab="mol/m3_phase"
  )
  legend(
    x = "bottom",
    legend = c("H2S", "HS-", "S2-"),
    lty = 1,
    col = c(rcols[3], rcols[6], rcols[1]),
    lwd = 2
  )
}
```

### Split dynamic model results

```{r def split_trans_results}
#| echo: true
#| code-fold: true

split_trans_results <- function(trans, grid, tableau_species) {

  res     <- list()
  N_grid  <- attr(trans, "dimens")
  N_spec  <- attr(trans, "nspec")

  # extract blocks

  ## time stamps
  times    <- trans[,1]
  N_times  <- length(times)

  ## species concentrations
  start <- 2
  end   <- 2 + N_grid * N_spec - 1

  res[["species"]] <- data.frame(
    name  = rep(attr(trans, "ynames"), each = N_times * N_grid),
    value = as.vector(trans[,start:end]),
    time  = rep(times, N_grid * N_spec),
    step  = rep(seq(1, N_times), N_grid * N_spec),
    depth = rep(rep(grid$x.mid, each = N_times), N_spec)
  )

  ## transport - tran$dC
  data <- trans[, grep("^transport.tran.*\\.dC.*$", colnames(trans))]
  
  res[["transport"]] <- data.frame(
    name  = rep(sub(".dC.*", "", sub("transport.tran.", "", colnames(data))), each = N_times),
    value = unlist(as.vector(data)),
    time  = rep(times, N_grid * N_spec),
    step  = rep(seq(1, N_times), N_grid * N_spec),
    depth = rep(rep(grid$x.mid, each = N_times), N_spec)
  )

  ## transport - fluxes
  data <- trans[, grep("^transport.tran.*\\.flux.*$", colnames(trans))]
  
  res[["fluxes"]] <- data.frame(
    name  = rep(sub(".flux.*", "", sub("transport.tran.", "", colnames(data))), each = N_times),
    value = unlist(as.vector(data)),
    time  = rep(times, N_spec * 2),
    step  = rep(seq(1, N_times), N_spec * 2),
    type  = rep(rep(c("up", "down"), each = N_times), N_spec)
  )

  ## sumR
  data   <- trans[, grep("^sumR.", colnames(trans))]

  N_sumR <- attr(trans, "lengthvar")[["sumR"]]
  len_to_remove <- nchar(as.character(rep(seq(1, N_grid), N_sumR)))
  names <- substr(colnames(data), 1, nchar(colnames(data)) - len_to_remove)
  names <- rep(sub("sumR.R.", "", names), each = N_times)

  res[["sumR"]] <- data.frame(
    name  = names,
    value = unlist(as.vector(data)),
    time  = rep(times, N_grid * N_sumR),
    step  = rep(seq(1, N_times), N_grid * N_sumR),
    depth = rep(rep(grid$x.mid, each = N_times), N_sumR)
  )

  ## reaction rates
  data <- trans[, grep("^rates.", colnames(trans))]

  N_rates <- attr(trans, "lengthvar")[["rates"]]
  len_to_remove <- nchar(as.character(rep(seq(1, N_grid), N_rates)))
  names <- substr(colnames(data), 1, nchar(colnames(data)) - len_to_remove)
  names <- rep(sub("rates.", "", names), each = N_times)
  
  res[["rates"]] <- data.frame(
    name  = names,
    value = unlist(as.vector(data)),
    time  = rep(times, N_grid * N_rates),
    step  = rep(seq(1, N_times), N_grid * N_rates),
    depth = rep(rep(grid$x.mid, each = N_times), N_rates)
  )

  ## omega
  data <- trans[, grep("^omega.", colnames(trans))]

  N_omega <- attr(trans, "lengthvar")[["omega"]]
  len_to_remove <- nchar(as.character(rep(seq(1, N_grid), N_omega)))
  names <- substr(colnames(data), 1, nchar(colnames(data)) - len_to_remove)
  names <- rep(sub("omega.", "", names), each = N_times)
  
  res[["omega"]] <- data.frame(
    name  = names,
    value = unlist(as.vector(data)),
    time  = rep(times, N_grid * N_omega),
    step  = rep(seq(1, N_times), N_grid * N_omega),
    depth = rep(rep(grid$x.mid, each = N_times), N_omega)
  )

  ## solute equilibrium: speciation
  data <- trans[, grep("^solute_equilibrium.species_conc", colnames(trans))]

  N_tableau_species <- length(tableau_species)

  res[["speciation"]] <- data.frame(
    name  = rep(tableau_species, each = N_grid * N_times),
    value = unlist(as.vector(data)),
    time  = rep(times, N_grid * N_tableau_species),
    step  = rep(seq(1, N_times), N_grid * N_tableau_species),
    depth = rep(rep(grid$x.mid, each = N_times), N_tableau_species)
  )

  return(res)

}
```

### Plot dynamic concentrations (contour)

```{r def plot_trans_conc_contour}
#| echo: true
#| code-fold: true

plot_trans_conc_contour <- function(trans_res, species) {

  data <- subset(trans_res$species, name == species)

  fig <- plot_ly(data, x = ~time, y = ~depth, z = ~value, type = "contour")

  fig <- fig %>% layout(
    title = list(text = paste(species, " (mol/(m3_phase))"), yanchor = "top", y = 1),
    xaxis = list(title = "time (yr)"),
    yaxis = list(title = "depth (m)", range = c(max(data$depth),0)),
    font = list(size = 16)
  )

  fig
}
```

### Plot dynamic concentrations (lines)

```{r def plot_trans_conc_lines}
#| echo: true
#| code-fold: true

plot_trans_conc_lines <- function(trans_res, species, timesteps) {

  data <- subset(trans_res$species, name == species)

  fig <- plot_ly(type = 'scatter', mode = 'lines', line = list(width = 3))

  for (timestep in timesteps) {
    depth <- subset(data, time == timestep)$depth
    value <- subset(data, time == timestep)$value
    fig   <- fig %>% add_trace(y = ~depth, x = ~value, name = toString(timestep))
  }

  fig <- fig %>% layout(
    xaxis = list(title = paste(species, " (mol/(m3_phase))")),
    yaxis = list(title = "depth (m)", range = c(max(data$depth),0)),
    font  = list(size = 16)
  )

  fig
}
```


## Model Setup

### Grid
```{r grid setup}
#| echo: true

# working: 0.5m, 1000 layers

length   <- 0.2  # (m)
N_grid   <- 400  # (-)

grid     <- setup.grid.1D(L = length, N = N_grid)
```

```{r save_grid}
saveRDS(grid, "last_results/grid.rds")
write.csv(grid$x.mid, "../temp_drop/grid.csv")
```

### Porosity / Solid Volume Fraction
```{r porosity}
#| echo: true

porFun.L  <- function(x, por.SWI, por.deep, porcoef) {
  return( por.deep + (por.SWI-por.deep) * exp(-x*porcoef) ) 
}

porFun.S  <- function(x, por.SWI, por.deep, porcoef) {
  return( 1 - porFun.L(x, por.SWI, por.deep, porcoef) ) 
}

por.swi  <- 0.8 #0.818
por.deep <- 0.8 #0.818

grid.por <- setup.prop.1D(func = porFun.L, grid = grid, por.SWI = por.swi,
                           por.deep=por.deep, porcoef=100)

grid.svf  <- setup.prop.1D(func = porFun.S, grid = grid, por.SWI = por.swi,
                           por.deep=por.deep, porcoef=100)

conversion_factors <- list(
  s2p = ((grid.svf$mid) / grid.por$mid), # conversion of x/m3_svf to x/m3_pw
  p2s = (grid.por$mid / (grid.svf$mid))  # conversion of x/m3_pw  to x/m3_svf
)
```

### Advective Velocities
```{r advection}
#| echo: true

# sedimentation velocity
v <- 0.002  # (m/yr)

# returns pore water advective velocities (u) and solid phase advective velocities (v)
advective_velocities <- setup.compaction.1D(
    v.0 = v,
    por.0 = por.swi,
    por.inf = por.deep,
    por.grid = grid.por
)
```

### Irrigation
```{r irrigation}
#| echo: true

#grid.k_iri <- setup.prop.1D(
#  func = p.exp,
#  grid = grid,
#  y.0 = 200, # yr-1
#  y.inf = 0, # yr-1
#  x.att = 0.035
#)

# k_iri = 0 @ 12cm
get_k_iri_func1 <- function() {
  data <- read.csv("./WangVanCapProfiles/WVC_irrigation_coefficient_zero.csv")
  data$depth <- data$depth * 0.01
  func <- approxfun(y = data$k, x = data$depth, rule = 2)
  return(func)
}

# original profile
get_k_iri_func2 <- function(grid) {
  
  data <- read.csv("./WangVanCapProfiles/WVC_irrigation_coefficient.csv")
  data$depth <- data$depth * 0.01

  fit <- nls(k ~ p.exp(depth, y.0, y.inf = 0, x.L = 0, x.att),
    data = data,
    start = list(y.0 = 133, x.att = 0.05)
  )

  func <- function(x) {
    p.exp(x, coefficients(fit)["y.0"], 0, 0, coefficients(fit)["x.att"])
  }

  return(func)
  
}

get_k_iri_func3 <- function(grid) {

  func <- function(x) {
    a <- 200
    b <- 0.28 * 100
    a * exp(-b * x)
  }

  return(func)
  
}

k_iri_func <- get_k_iri_func3()

grid.k_iri <- setup.prop.1D(
  func = k_iri_func,
  grid = grid
)
```



### Diffusion Coefficients
```{r diffusion}
#| echo: true

salinity    <- 35 # (psu)
temperature <- 5  # (dgC)
pressure    <- 1.013253 # (bar)

grid.Db <- setup.prop.1D(
    func = function(x){ifelse(x <= 0.2, 0.002, 0)}, # 0.002
    grid = grid
)

#grid.Db <- setup.prop.1D(
#         func = p.sig,
#         y.0 = 0.002,
#         y.inf = 0.0002,
#         x.L = 0.2,
#         x.att = 0.05,
#         grid = grid
#)

tortuosity_squared <- 1 - log(grid.por$int^2)

s2yr <- 3600 * 24 * 365.25

diffcoeff_species <- c("HCO3", "HCO3", "CH4", "H2PO4", "NO3", "NH4", "SO4",
                       "O2", "Mn", "Fe", "H2S", "N2", "Ca")

diffcoeffs <- diffcoeff(
  species = diffcoeff_species,
  S = salinity,
  t = temperature,
  P = pressure
)

diffusion_coefficients <- list(
  
  # solute: molar diffusion coefficient / tortuosity**2 + bioturbation coefficient
  # solid: bioturbation coefficient
  
  ALK   = diffcoeffs[[1]]  * s2yr / tortuosity_squared + grid.Db$int,
  DIC   = diffcoeffs[[2]]  * s2yr / tortuosity_squared + grid.Db$int,
  CH4   = diffcoeffs[[3]]  * s2yr / tortuosity_squared + grid.Db$int,
  H2PO4 = diffcoeffs[[4]]  * s2yr / tortuosity_squared + grid.Db$int,
  NO3   = diffcoeffs[[5]]  * s2yr / tortuosity_squared + grid.Db$int,
  NH4   = diffcoeffs[[6]]  * s2yr / tortuosity_squared + grid.Db$int,
  SO4   = diffcoeffs[[7]]  * s2yr / tortuosity_squared + grid.Db$int,
  O2    = diffcoeffs[[8]]  * s2yr / tortuosity_squared + grid.Db$int,
  Mn2   = diffcoeffs[[9]]  * s2yr / tortuosity_squared + grid.Db$int,
  Fe2   = diffcoeffs[[10]] * s2yr / tortuosity_squared + grid.Db$int,
  H2S   = diffcoeffs[[11]] * s2yr / tortuosity_squared + grid.Db$int,
  N2    = diffcoeffs[[12]] * s2yr / tortuosity_squared + grid.Db$int,
  Ca2   = diffcoeffs[[13]] * s2yr / tortuosity_squared + grid.Db$int,
  solid = grid.Db$int
    
)
```

```{r plot por, Db, ...}
#| layout-ncol: 3
#| fig.height: 3.5
#| fig.width: 3.5

plot(x=grid.por$mid, y=grid$x.mid, type="l",
     ylim=c(length,0), ylab="depth (m)", xlab="porosity (m3_liq/m3_bulk)")
plot(x=diffusion_coefficients$solid, y=grid$x.int, type="l",
     ylim=c(length,0), ylab="depth (m)", xlab="D_solid (m2/yr)")
plot(x=diffusion_coefficients$DIC, y=grid$x.int, type="l",
     ylim=c(length,0), ylab="depth (m)", xlab="D_DIC (m2/yr)")
plot(x=diffusion_coefficients$O2, y=grid$x.int, type="l",
     ylim=c(length,0), ylab="depth (m)", xlab="D_O2 (m2/yr)")
plot(x=grid.k_iri$int, y=grid$x.int, type="l",
     ylim=c(length,0), ylab="depth (m)", xlab="k_iri int (yr-1)")
```

### Boundary Conditions
```{r boundary conditions}
#| echo: true

boundary_conditions <- c(
  # Alkalinity in eq/m3?
  ALK_up   = 2.5, #!
  # Solutes: concentrations in mol m-3
  DIC_up   = 2.56,
  CH4_up   = 0,
  H2PO4_up = 0.00235,
  NO3_up   =  0.0015, #!
  NH4_up   = 0.022, #!
  SO4_up   = 28, #!
  O2_up    = 0.213, #!
  N2_up    = 0,
  Mn2_up   = 0.002, #!
  Fe2_up   = 0, #!
  H2S_up   = 0, #!
  Ca2_up   = 1.17,
  # Solids: flux in mol m-2 yr-1
  OM_up      = 0.019 * 365, #0.013
  OM_beta_up = 0,
  MnO2_up    = 0.4, #!
  FeOH3_up   = 0.75, #!
  FeOH3_P_up = 0,
  FeCO3_up   = 0,
  S0_up      = 0, #!
  FeS_up     = 0,
  Viv_up     = 0,
  CaCO3_up   = 0,
  Apatite_up = 0,
  MnCO3_up   = 0
)
```

### Reaction Parameters
```{r reaction parameters}
#| echo: true

dry_density_sediment <- 2400#2700 # kg/m³

reaction_parameter <- c(
    # OM degradation
  ## reaction constants
  k_alpha = 2.2e-3 * 365, # yr-1; 0.005 * 365
  k_beta  = 0 * 6e-5 * 365, # yr-1
  ## composition of OM
  OM_a    = 1,
  OM_b    = 1 / 8.8,
  OM_c    = 1 / 106,
  ## limitation / inhibition constants (mol m-3)
  K_O2    = 0.02,
  K_NO3   = 0.002,
  K_MnO2  = 0.016 * dry_density_sediment, # 0.016 | 0.004
  K_FeOH3 = 0.1 * dry_density_sediment, # 0.1 | 0.065
  K_SO4   = 1.6,
  K_H3PO4 = 10^-5,
  ## ratio of Fe to P in Fe(OH)_3(H3PO4)_lambda
  lambda  = 0.6,

  # Siderite Precipitation / Dissolution
  k_precip_siderite = 1215,   # mol m-3_sf yr-1
  k_diss_siderite   = 2.5e-1, # yr-1
  Ksp_siderite      = 10^(-8.4) * 1000^2, # mol2 m-6 # *0.6
  #k_precip_siderite = 1.8e2,  # mol m-3_sf yr-1
  #k_diss_siderite   = 2.5e-1, # yr-1
  #Ksp_siderite      = 10^(-10.4) * 1000^2, # mol2 m-6
  

  # FeOH3 / FeOH3~H3PO4-Reduction coupled to Sulphide Oxidation
  k_Fe_red_S_ox = 8, # m3 mol-1 yr-1

  # FeOH3 / FeOH3~H3PO4-Formation
  k_FeOH3_formation = 1.4e5, # m3 mol-1 yr-1

  # H2S Oxidation
  k_H2S_oxidation = 1.6e2, # m3_pw mol-1 yr-1

  # SO4 Reduction Coupled to AOM
  k_SO4_reduction = 10, # m3_pw mol-1 yr-1

  # FeS Precipitation / Dissolution
  k_precip_FeS = 40.5, # mol m-3_sf yr-1
  k_diss_FeS   = 1 * 1e-3, # yr-1
  Ksp_FeS      = 10^(-2.2) * 1000, # mol m-3 # *100
  #k_precip_FeS = 1 * 2.5e-3, # mol m-3_sf yr-1
  #k_diss_FeS   = 1 * 1e-3,   # yr-1
  #Ksp_FeS      = 10^(-3.915) * 1000, # mol m-3

  # Vivianite Precipitation / Dissolution
  k_precip_Viv = 0 * 1.1e-2, # mol m-3_sf yr-1
  k_diss_Viv   = 0 * 5.3e-3, # yr-1
  Ksp_Viv      = 10^(-36) * 1000^5, # mol3 m-9

  # CaCO3 Precipitation / Dissolution
  k_precip_CaCO3 = 0 * 1.1e-1,  # mol m-3_sf yr-1
  k_diss_CaCO3   = 0 * 1.25e-1, # mol m-3_sf yr-1
  Ksp_CaCO3      = 10^(-8.48) * 1000^2, # mol2 m-6

  # Apatite Precipitation / Dissolution
  k_precip_Apatite = 0 * 1.4e-6, # mol m-3 yr-1
  k_diss_Apatite   = 0 * 3.7e-2, # yr-1
  Ksp_Apatite      = 6e-20,  # mol3 m-9; 6e-20

  # Oxidation of Fe2 by MnO2
  k_I11 = 3e3,

  # Ammonium Oxidation
  k_I12 = 5e3,

  # Sulphide Oxidation by MnO2
  k_I14 = 20,

  # FeS Oxidation by O2
  k_I16 = 3.2e2,

  # CH4 Oxidation by O2
  k_I17 = 1e7,

  # MnCO3 Precipitation / Dissolution
  k_precip_MnCO3 = 270,
  k_diss_MnCO3 = 0.8,
  Ksp_MnCO3 = 10^(-8.5) * 1000^2, # *0.15

  # Oxidation of sorbed Fe2
  k_I10 = 5e4,

  # Oxidation of sorbed Mn2
  k_I8 = 5e3
)
```

### Fixed OM Degradation Rate
```{r fixed OM Degradation Rate}
#| echo: true

get_OMdeg_total1 <- function(grid, grid.svf) {
  
  data <- read.csv("./WangVanCapProfiles/WVC_OMdeg_rate.csv")

  data$depth <- data$depth * 0.01 # cm to m

  # fit exponential model
  fit <- nls(rate ~ p.exp(depth, y.0, y.inf = 0, x.L = 0, x.att),
    data = data,
    start = list(y.0 = 133, x.att = 0.05)
  )

  # calculate OM degradation rate at grid mids
  OMdeg_total <- p.exp(grid$x.mid, coefficients(fit)["y.0"], 0, 0, coefficients(fit)["x.att"])
  # correct unit from umol cm^-3_total yr-1 to mol m^-3_svf yr-1
  OMdeg_total <- OMdeg_total * 1e-6 * 100^3 * 1 / grid.svf$mid

  return(OMdeg_total)
  
}

get_OMdeg_total2 <- function(grid, grid.svf) {

  R_OMdeg_tot <- function(x) {
    R0 <- 135
    b  <- 0.208
    R0 * exp(-b * x)
  }

  # calculate OM degradation rate at grid mids
  OMdeg_total <- R_OMdeg_tot(grid$x.mid * 100)
  # correct unit from umol cm^-3_total yr-1 to mol m^-3_svf yr-1
  OMdeg_total <- OMdeg_total * 1e-6 * 100^3 * 1 / grid.svf$mid

  return(OMdeg_total)
  
}

OMdeg_total <- get_OMdeg_total2(grid, grid.svf)
```

### Fixed sorbed Fe2
```{r fixed sorbed Fe2}
#| echo: true

get_Fe2_sorbed <- function(grid, grid.svf) {
  
  data <- read.csv("./WangVanCapProfiles/WVC_Fe2_sorbed.csv")

  data$depth <- data$depth * 0.01 # cm to m
  data$conc  <- data$conc * 1e-6 * 100^3 * 1 # umol/cm3 to mol/m3

  # calculate OM degradation rate at grid mids
  func <- approxfun(y = data$conc, x = data$depth, rule = 2)

  Fe2_sorbed <- func(grid$x.mid)

  # convert from mol m^-3 to mol m-3_sf
  #Fe2_sorbed <- Fe2_sorbed * 1 / grid.svf$mid

  return(Fe2_sorbed)
  
}

Fe2_sorbed <- get_Fe2_sorbed(grid, grid.svf)
```

### Fixed sorbed Mn2
```{r fixed sorbed Mn2}
#| echo: true

get_Mn2_sorbed <- function(grid) {
  
  Mn2_sorbed <- setup.prop.1D(
    func = function(x){ifelse(x >= 0.005 & x <= 0.02, 0, 0)},
    grid = grid
  )

  return(Mn2_sorbed$mid)
  
}

Mn2_sorbed <- 0#get_Mn2_sorbed(grid)
```


### Collect Inputs
```{r collect inputs}
#| echo: true

#ratios_pH7 <- speciation_ratios(pH = 7, tableau, tableau_species, logK)
pH_profile <- read.csv("./WangVanCapProfiles/WVC_pH_profile.csv")
colnames(pH_profile) <- c("pH", "depth")
pH_profile$depth <- pH_profile$depth * 0.01
pH_in_layers <- approx(y = pH_profile$pH, x = pH_profile$depth, xout = grid$x.mid, rule = 2)
srm <- matrix(ncol = N_grid, nrow = 13)
for (i in seq(1, N_grid)) {
  srm[,i] <- speciation_ratios(pH = pH_in_layers$y[i], tableau, tableau_species, logK)
}

ratios <- list(
    fixed_H = as.vector(srm[1,]),
    f_H2CO3 = as.vector(srm[2,]),
    f_HCO2  = as.vector(srm[3,]),
    f_CO3   = as.vector(srm[4,]),
    f_H2S   = as.vector(srm[5,]),
    f_HS    = as.vector(srm[6,]),
    f_S2    = as.vector(srm[7,]),
    f_H3PO4 = as.vector(srm[8,]),
    f_H2PO4 = as.vector(srm[9,]),
    f_HPO4  = as.vector(srm[10,]),
    f_PO4   = as.vector(srm[11,]),
    f_NH4   = as.vector(srm[12,]),
    f_NH3   = as.vector(srm[13,])
  )


parameter <- c(
  N_grid = N_grid,
  conversion_factors,
  boundary_conditions,
  reaction_parameter,
  ratios,
  logK = logK,
  tableau = tableau,
  tableau_species = tableau_species,
  OMdeg_total = OMdeg_total,
  Fe2_sorbed = Fe2_sorbed,
  Mn2_sorbed = Mn2_sorbed
)

N_species <- length(model_species)
```

### Model Function
```{r model function}
#| echo: true
#| code-fold: show

model <- function (t, state, parms, diff_coeffs, adv_vel, k_iri, solve_equilibrium, precipitation){
  
  with (as.list(parms),{
    
    # assign state variables
    i <- 1
    # state variables for solutes are total amounts,
    # but implicitly concentrations (total_amount/m³_pw)
    ALK     <- state[((i-1) * N_grid + 1) : (i * N_grid)]; i <- i + 1 # -TOT_H
    DIC     <- state[((i-1) * N_grid + 1) : (i * N_grid)]; i <- i + 1 # TOT_H2CO3
    TOT_P   <- state[((i-1) * N_grid + 1) : (i * N_grid)]; i <- i + 1
    TOT_N5  <- state[((i-1) * N_grid + 1) : (i * N_grid)]; i <- i + 1
    TOT_N3 <- state[((i-1) * N_grid + 1) : (i * N_grid)]; i <- i + 1
    TOT_S6  <- state[((i-1) * N_grid + 1) : (i * N_grid)]; i <- i + 1
    TOT_S2  <- state[((i-1) * N_grid + 1) : (i * N_grid)]; i <- i + 1
    CH4     <- state[((i-1) * N_grid + 1) : (i * N_grid)]; i <- i + 1
    O2      <- state[((i-1) * N_grid + 1) : (i * N_grid)]; i <- i + 1
    N2      <- state[((i-1) * N_grid + 1) : (i * N_grid)]; i <- i + 1
    Mn2     <- state[((i-1) * N_grid + 1) : (i * N_grid)]; i <- i + 1
    Fe2     <- state[((i-1) * N_grid + 1) : (i * N_grid)]; i <- i + 1
    Ca2     <- state[((i-1) * N_grid + 1) : (i * N_grid)]; i <- i + 1
    # state variables for solids are tableau-species concentrations (mol/m3_svf)
    MnO2    <- state[((i-1) * N_grid + 1) : (i * N_grid)]; i <- i + 1
    OM      <- state[((i-1) * N_grid + 1) : (i * N_grid)]; i <- i + 1
    OM_beta <- state[((i-1) * N_grid + 1) : (i * N_grid)]; i <- i + 1
    FeOH3   <- state[((i-1) * N_grid + 1) : (i * N_grid)]; i <- i + 1
    FeOH3_P <- state[((i-1) * N_grid + 1) : (i * N_grid)]; i <- i + 1
    FeCO3   <- state[((i-1) * N_grid + 1) : (i * N_grid)]; i <- i + 1
    S0      <- state[((i-1) * N_grid + 1) : (i * N_grid)]; i <- i + 1
    FeS     <- state[((i-1) * N_grid + 1) : (i * N_grid)]; i <- i + 1
    Viv     <- state[((i-1) * N_grid + 1) : (i * N_grid)]; i <- i + 1
    CaCO3   <- state[((i-1) * N_grid + 1) : (i * N_grid)]; i <- i + 1
    Apatite <- state[((i-1) * N_grid + 1) : (i * N_grid)]; i <- i + 1
    MnCO3   <- state[((i-1) * N_grid + 1) : (i * N_grid)]; i <- i + 1

    # NH4 Adsorption
    #TOT_N3  <- TOT_N3_ads_and_free / (1 + 1.4)
    #NH4_ads <- (TOT_N3_ads_and_free - TOT_N3) * p2s
    NH4_ads  <- p2s * 1.4 * TOT_N3

    # Fe2 adsorption
    Fe2 <- Fe2 - Fe2_sorbed * s2p
    
    # transport terms
    ## weight used in the finite difference scheme for advection, defined on grid cell interfaces
    ## backward = 1, centred = 0.5, forward = 0
    AFDW <- 1 # << no visual effect

    tran.ALK     <- tran.1D(C = ALK, C.up = ALK_up, D = diff_coeffs$ALK, 
                            v = adv_vel$u, VF = grid.por, dx = grid, AFDW = AFDW)
    
    tran.DIC     <- tran.1D(C = DIC, C.up = DIC_up, D = diff_coeffs$DIC, 
                            v = adv_vel$u, VF = grid.por, dx = grid, AFDW = AFDW)
    
    tran.TOT_P   <- tran.1D(C = TOT_P, C.up = H2PO4_up, D = diff_coeffs$H2PO4, 
                            v = adv_vel$u, VF = grid.por, dx = grid, AFDW = AFDW)
    
    tran.TOT_N5  <- tran.1D(C = TOT_N5, C.up = NO3_up, D = diff_coeffs$NO3, 
                            v = adv_vel$u, VF = grid.por, dx = grid, AFDW = AFDW)
    
    tran.TOT_N3  <- tran.1D(C = TOT_N3, C.up = NH4_up, D = diff_coeffs$NH4, 
                            v = adv_vel$u, VF = grid.por, dx = grid, AFDW = AFDW)
    
    tran.TOT_S6  <- tran.1D(C = TOT_S6, C.up = SO4_up, D = diff_coeffs$SO4, 
                            v = adv_vel$u, VF = grid.por, dx = grid, AFDW = AFDW)
    
    tran.TOT_S2  <- tran.1D(C = TOT_S2, C.up = H2S_up, D = diff_coeffs$H2S, 
                            v = adv_vel$u, VF = grid.por, dx = grid, AFDW = AFDW)
    
    tran.CH4     <- tran.1D(C = CH4, C.up = CH4_up, D = diff_coeffs$CH4, 
                            v = adv_vel$u, VF = grid.por, dx = grid, AFDW = AFDW)

    tran.O2      <- tran.1D(C = O2, C.up = O2_up, D = diff_coeffs$O2, 
                            v = adv_vel$u, VF = grid.por, dx = grid, AFDW = AFDW)
    
    tran.N2      <- tran.1D(C = N2, C.up = N2_up, D = diff_coeffs$N2, 
                            v = adv_vel$u, VF = grid.por, dx = grid, AFDW = AFDW)
    
    tran.Mn2     <- tran.1D(C = Mn2, C.up = Mn2_up, D = diff_coeffs$Mn2, 
                            v = adv_vel$u, VF = grid.por, dx = grid, AFDW = AFDW)
    
    tran.Fe2     <- tran.1D(C = Fe2, C.up = Fe2_up, D = diff_coeffs$Fe2, 
                            v = adv_vel$u, VF = grid.por, dx = grid, AFDW = AFDW)

    tran.Ca2     <- tran.1D(C = Ca2, C.up = Ca2_up, D = diff_coeffs$Ca2, 
                            v = adv_vel$u, VF = grid.por, dx = grid, AFDW = AFDW)
    
    tran.MnO2    <- tran.1D(C = MnO2, flux.up = MnO2_up, D = diff_coeffs$solid, 
                            v = adv_vel$v, VF = grid.svf, dx = grid, AFDW = AFDW)
    
    tran.OM      <- tran.1D(C = OM, flux.up = OM_up, D = diff_coeffs$solid, 
                            v = adv_vel$v, VF = grid.svf, dx = grid, AFDW = AFDW)

    tran.OM_beta <- tran.1D(C = OM_beta, flux.up = OM_beta_up, D = diff_coeffs$solid, 
                            v = adv_vel$v, VF = grid.svf, dx = grid, AFDW = AFDW)
    
    tran.FeOH3   <- tran.1D(C = FeOH3, flux.up = FeOH3_up, D = diff_coeffs$solid, 
                            v = adv_vel$v, VF = grid.svf, dx = grid, AFDW = AFDW)
    
    tran.FeOH3_P <- tran.1D(C = FeOH3_P, flux.up = FeOH3_P_up, D = diff_coeffs$solid, 
                            v = adv_vel$v, VF = grid.svf, dx = grid, AFDW = AFDW)
    
    tran.FeCO3   <- tran.1D(C = FeCO3, flux.up = FeCO3_up, D = diff_coeffs$solid, 
                            v = adv_vel$v, VF = grid.svf, dx = grid, AFDW = AFDW)

    tran.S0      <- tran.1D(C = S0, flux.up = S0_up, D = diff_coeffs$solid, 
                            v = adv_vel$v, VF = grid.svf, dx = grid, AFDW = AFDW)

    tran.FeS     <- tran.1D(C = FeS, flux.up = FeS_up, D = diff_coeffs$solid, 
                            v = adv_vel$v, VF = grid.svf, dx = grid, AFDW = AFDW)
    
    tran.Viv     <- tran.1D(C = Viv, flux.up = Viv_up, D = diff_coeffs$solid, 
                            v = adv_vel$v, VF = grid.svf, dx = grid, AFDW = AFDW)

    tran.CaCO3   <- tran.1D(C = CaCO3, flux.up = CaCO3_up, D = diff_coeffs$solid, 
                            v = adv_vel$v, VF = grid.svf, dx = grid, AFDW = AFDW)

    tran.Apatite <- tran.1D(C = Apatite, flux.up = Apatite_up, D = diff_coeffs$solid, 
                            v = adv_vel$v, VF = grid.svf, dx = grid, AFDW = AFDW)

    tran.MnCO3   <- tran.1D(C = MnCO3, flux.up = MnCO3_up, D = diff_coeffs$solid, 
                            v = adv_vel$v, VF = grid.svf, dx = grid, AFDW = AFDW)

    tran.NH4_ads <- tran.1D(C = NH4_ads, flux.up = 0, D = diff_coeffs$solid, 
                            v = adv_vel$v, VF = grid.svf, dx = grid, AFDW = AFDW)
    
    # calculate solute equilibrium species (solve the tableau)
    if (solve_equilibrium) {
      # 1) grep the solute total components; here we need TOT_H = - ALK
      solute_totals <- c(-ALK, DIC, TOT_P, TOT_N5, TOT_N3, TOT_S6, TOT_S2)
      # only DIC and H2S considered in Wang and Van Capellen
      #solute_totals <- c(-ALK, DIC, rep(0, N_grid * 4), TOT_S2)
      # 2) solve tableau
      solute_equilibrium  <- solve_tableau(solute_totals, tableau, logK, N_grid, tableau_species)
      # 3) assign results
      H     <- solute_equilibrium$species_conc[,1]
      pH    <- -log10(H * 10^-3)
      OH    <- solute_equilibrium$species_conc[,2]
      H2CO3 <- solute_equilibrium$species_conc[,3]
      HCO3  <- solute_equilibrium$species_conc[,4]
      CO3   <- solute_equilibrium$species_conc[,5]
      H3PO4 <- solute_equilibrium$species_conc[,6]
      H2PO4 <- solute_equilibrium$species_conc[,7]
      HPO4  <- solute_equilibrium$species_conc[,8]
      PO4   <- solute_equilibrium$species_conc[,9]
      HNO3  <- solute_equilibrium$species_conc[,10]
      NO3   <- solute_equilibrium$species_conc[,11]
      NH4   <- solute_equilibrium$species_conc[,12]
      NH3   <- solute_equilibrium$species_conc[,13]
      H2SO4 <- solute_equilibrium$species_conc[,14]
      HSO4  <- solute_equilibrium$species_conc[,15]
      SO4   <- solute_equilibrium$species_conc[,16]
      H2S   <- solute_equilibrium$species_conc[,17]
      HS    <- solute_equilibrium$species_conc[,18]
      S2    <- solute_equilibrium$species_conc[,19]
    } else {
      solute_equilibrium <- NA
    }
    
    # FeOH3 fractions
    FeOH3_tot <- FeOH3 + FeOH3_P
    chi <- rep(1, length(FeOH3))
    ind_valid <- FeOH3_tot > 0
    chi[ind_valid] <- FeOH3[ind_valid] / FeOH3_tot[ind_valid]
    
    # limitation & inhibition terms
    #L_O2    <- O2   / (K_O2 + O2)
    #I_O2    <- K_O2 / (K_O2 + O2)
    
    #L_NO3   <- TOT_N5 / (K_NO3 + TOT_N5)
    #I_NO3   <- K_NO3 / (K_NO3 + TOT_N5)
    
    #L_MnO2  <- MnO2   / (K_MnO2 + MnO2)
    #I_MnO2  <- K_MnO2 / (K_MnO2 + MnO2)
    
    #L_FeOH3 <- FeOH3_tot / (K_FeOH3 + FeOH3_tot)
    #I_FeOH3 <- K_FeOH3 / (K_FeOH3 + FeOH3_tot)
    
    #L_SO4   <- TOT_S6 / (K_SO4 + TOT_S6)
    #I_SO4   <- K_SO4 / (K_SO4 + TOT_S6)
    
    L_H3PO4 <- TOT_P   / (K_H3PO4 + TOT_P)
    I_H3PO4 <- K_H3PO4 / (K_H3PO4 + TOT_P)

    # linearized limitation & inhibition terms
    L_O2    <- pmin(1, O2 / K_O2)
    I_O2    <- 1 - L_O2
    
    L_NO3   <- pmin(1, TOT_N5 / K_NO3)
    I_NO3   <- 1 - L_NO3
    
    L_MnO2  <- pmin(1, MnO2 / K_MnO2)
    I_MnO2  <- 1 - L_MnO2

    L_FeOH3 <- pmin(1, FeOH3_tot / K_FeOH3)
    I_FeOH3 <- 1 - L_FeOH3
    
    L_SO4   <- pmin(1, TOT_S6 / K_SO4)
    I_SO4   <- 1 - L_SO4
    
    # Reaction Rates
    ## OM Degradation
    #R_OMdeg_tot <- k_alpha * OM
    R_OMdeg_tot <- OMdeg_total # fixed rate profile from Wang & Van Capellen

    R1 <- R_OMdeg_tot * L_O2
    R2 <- R_OMdeg_tot * L_NO3                            * I_O2
    R3 <- R_OMdeg_tot * L_MnO2                   * I_NO3 * I_O2
    R4 <- R_OMdeg_tot * L_FeOH3         * I_MnO2 * I_NO3 * I_O2
    R5 <- R_OMdeg_tot * L_SO4 * I_FeOH3 * I_MnO2 * I_NO3 * I_O2
    R6 <- R_OMdeg_tot * I_SO4 * I_FeOH3 * I_MnO2 * I_NO3 * I_O2

    R1b <- k_beta * OM_beta * L_O2
    R2b <- k_beta * OM_beta * L_NO3                            * I_O2
    R3b <- k_beta * OM_beta * L_MnO2                   * I_NO3 * I_O2
    R4b <- k_beta * OM_beta * L_FeOH3         * I_MnO2 * I_NO3 * I_O2
    R5b <- k_beta * OM_beta * L_SO4 * I_FeOH3 * I_MnO2 * I_NO3 * I_O2
    R6b <- k_beta * OM_beta * I_SO4 * I_FeOH3 * I_MnO2 * I_NO3 * I_O2

    ## Siderite: R7 = precipitation, R8 = dissolution
    if (precipitation & solve_equilibrium) {
      omega_siderite <- (Fe2 * CO3) / Ksp_siderite
    } else if (precipitation) {
      omega_siderite <- (Fe2 * (DIC * f_CO3)) / Ksp_siderite
    } else {
      omega_siderite <- rep(1, N_grid)
    }
    R7 <- ifelse(omega_siderite > 1, k_precip_siderite * (omega_siderite - 1), 0)
    R8 <- ifelse(omega_siderite < 1, k_diss_siderite * FeCO3 * (1 - omega_siderite), 0)

    ## FeOH3 / FeOH3~H3PO4-Reduction coupled to Sulphide Oxidation
    R9 <- k_Fe_red_S_ox * FeOH3_tot * TOT_S2

    ## FeOH3 / FeOH3~H3PO4-Formation
    R10 <- k_FeOH3_formation * O2 * Fe2

    ## H2S Oxidation
    R11 <- k_H2S_oxidation * O2 * TOT_S2

    ## SO4-Reduction Coupeld to AOM
    if (solve_equilibrium) {
      R12 <- k_SO4_reduction * CH4 * SO4
    } else {
      R12 <- k_SO4_reduction * CH4 * (TOT_S6 * 1)
    }

    ## FeS: R13 = precipitation, R14 = dissolution
    if (precipitation & solve_equilibrium) {
      omega_FeS <- (Fe2 * HS / H) / Ksp_FeS
    } else if (precipitation) {
      omega_FeS <- (Fe2 * (TOT_S2 * f_HS) / fixed_H) / Ksp_FeS
    } else {
      omega_FeS <- rep(1, N_grid)
    }
    R13 <- ifelse(omega_FeS > 1, k_precip_FeS * (omega_FeS - 1), 0)
    R14 <- ifelse(omega_FeS < 1, k_diss_FeS * FeS * (1 - omega_FeS), 0)

    ## Vivianite: R15 = precipitation, R16 = dissolution
    if (precipitation & solve_equilibrium) {
      omega_Viv <- (Fe2 * PO4^(2/3)) / Ksp_Viv^(1/3)
    } else if (precipitation) {
      omega_Viv <- (Fe2 * (TOT_P * f_PO4)^(2/3)) / Ksp_Viv^(1/3)
    } else {
      omega_Viv <- rep(1, N_grid)
    }
    R15 <- ifelse(omega_Viv > 1, k_precip_Viv * (omega_Viv - 1)^2, 0)
    R16 <- ifelse(omega_Viv < 1, k_diss_Viv * Viv * (1 - omega_Viv)^2, 0)

    ## CaCO3: R17 = precipitation, R18 = dissolution
    if (precipitation & solve_equilibrium) {
      omega_CaCO3 <- (Ca2 * CO3) / Ksp_CaCO3
    } else if (precipitation) {
      omega_CaCO3 <- (Ca2 * (DIC * f_CO3)) / Ksp_CaCO3
    } else {
      omega_CaCO3 <- rep(1, N_grid)
    }
    R17 <- ifelse(omega_CaCO3 > 1, k_precip_CaCO3 * (omega_CaCO3 - 1)^2, 0)
    R18 <- ifelse(omega_CaCO3 < 1, k_diss_CaCO3 * CaCO3 * (1 - omega_CaCO3)^2, 0)

    ## Apatite: R19 = precipitation, R20 = dissolution
    if (precipitation & solve_equilibrium) {
      omega_Apatite <- (Ca2 * HPO4^(2/3)) / (Ksp_Apatite^(1/3) / H^(2/3))
    } else if (precipitation) {
      omega_Apatite <- (Ca2 * (TOT_P * f_HPO4)^(2/3)) / (Ksp_Apatite^(1/3) / fixed_H^(2/3))
    } else {
      omega_Apatite <- rep(1, N_grid)
    }
    R19 <- ifelse(omega_Apatite > 1, k_precip_Apatite * (omega_Apatite - 1)^2, 0)
    R20 <- ifelse(omega_Apatite < 1, k_diss_Apatite * Apatite * (1 - omega_Apatite)^2, 0)

    ## Fe Oxidation by MnO2
    R21 <- k_I11 * MnO2 * Fe2

    ## Ammonium Oxidation
    if (solve_equilibrium) {
      R22 <- k_I12 * NH4 * O2
    } else {
      R22 <- k_I12 * TOT_N3 * f_NH4 * O2
    }

    ## Sulphide Oxidation by MnO2
    R23 <- k_I14 * TOT_S2 * MnO2

    ## FeS Oxidation by O2
    R24 <- k_I16 * FeS * O2

    ## CH4 Oxidation by O2
    R25 <- k_I17 * CH4 * O2

    ## MnCO3: R26 = precipitation, R27 = dissolution
    if (precipitation & solve_equilibrium) {
      omega_MnCO3 <- (Mn2 * CO3) / Ksp_MnCO3
    } else if (precipitation) {
      omega_MnCO3 <- (Mn2 * (DIC * f_CO3)) / Ksp_MnCO3
    } else {
      omega_MnCO3 <- rep(1, N_grid)
    }
    R26 <- ifelse(omega_MnCO3 > 1, k_precip_MnCO3 * (omega_MnCO3 - 1), 0)
    R27 <- ifelse(omega_MnCO3 < 1, k_diss_MnCO3 * MnCO3 * (1 - omega_MnCO3), 0)

    # Oxidation of sorbed Fe2
    R28 <- k_I10 * Fe2_sorbed * O2

    # Oxidation of sorbed Mn2
    R29 <- k_I8 * Mn2_sorbed * O2

    
    # change of species concentration due to reactions
    ## solutes step1: change of slow-reaction-species (srs), the ones for which
    ## the reactions are formulated, according to reaction stoichiometry
    R.srs_O2    <- (- s2p * (OM_a + 2 * OM_b) * R1 - s2p * R1b # + 2 * OM_b according to Wang and Van Cap
                    - 1/4 * R10
                    - 2 * R11
                    - 2 * R22
                    - 2 * s2p * R24
                    - 2 * R25
                    - 1/4 * s2p * R28
                    - 1/2 * s2p * R29)
    R.srs_CO2   <- (s2p * (R1 + R2 + R3 + R4 + R5 + 1/2 * R6)
                    + s2p * (R1b + R2b + R3b + R4b + R5b + 1/2 * R6b)
                    - s2p * R7 + s2p * R8  # Siderite Precipitation / Dissolution
                    + 1 * R12
                    - s2p * R17 + s2p * R18
                    + R25
                    - s2p * R26 + s2p * R27)
    R.srs_NH3   <- (OM_b * s2p * (R3 + R4 + R5 + R6) # R1 removed according to Wang and Van Capellen -> NO3 is produced in R1; R2 removed -> N2 produced
                    + OM_b * s2p * (R1b + R2b + R3b + R4b + R5b + R6b)
                    - R22)
    R.srs_H3PO4 <- (s2p * ( OM_c * (R1 + R2 + R3 + R5 + R6) +
                      (OM_c + (1 - chi) * 4 * lambda) * R4 )
                    + s2p * ( OM_c * (R1b + R2b + R3b + R5b + R6b) +
                      (OM_c + (1 - chi) * 4 * lambda) * R4b )
                    + 2 * (1 - chi) * lambda * s2p * R9
                    - 4/4 * lambda * L_H3PO4 * R10
                    - 2 * s2p * R15 + 2 * s2p * R16
                    - 2 * s2p * R19 + 2 * s2p * R20)
    R.srs_HNO3  <- (- (4 * OM_a + 3 * OM_b) / 5 * s2p * R2 - 4/5 * s2p * R2b # adjusted R2 (Wang and Van Capellen)
                     + R22
                     + OM_b * s2p * R1 # NO3 produced in R1 (Wang and Van Capellen)
                   )
    R.srs_N2    <- (2 * OM_a + 4 * OM_b)/5 * s2p * R2 + 2/5 * s2p * R2b # R2 adjusted (Wang and Van Capellen)
    R.srs_FeOH2 <- (4 * s2p * R4 + 4 * s2p * R4b # OM degradation
                    - s2p * R7 + s2p * R8        # Siderite Precipitation / Dissolution
                    + 2 * s2p * R9
                    - 4/4 * R10
                    - s2p * R13 + s2p * R14
                    - 3 * s2p * R15 + 3 * s2p * R16
                    - 2 * s2p * R21
                    + s2p * R24)
    R.srs_MnOH2 <- (2 * s2p * R3 + 2 * s2p * R3b
                    + s2p * R21
                    + s2p * R23
                    - s2p * R26 + s2p * R27)
    R.srs_H2SO4 <- (- 1/2 * s2p * R5 - 1/2 * s2p * R5b
                    + 1 * R11
                    - 1 * R12
                    + s2p * R24)
    R.srs_H2S   <- (1/2 * s2p * R5 + 1/2 * s2p * R5b
                    - 1 * s2p * R9
                    - 1 * R11
                    + 1 * R12
                    - s2p * R13 + s2p * R14
                    - s2p * R23)
    R.srs_CH4   <- (1/2 * s2p * R6 + 1/2 * s2p * R6b
                    - 1 * R12
                    - R25)
    R.srs_CaOH2 <- (- s2p * R17 + s2p * R18
                    - 3 * s2p * R19 + 3 * s2p * R20)
    
    ## solutes step2: change of tableau-components = model species
    ## translation of change of slow-reaction-species to change of 
    ## tableau components using the translation table
    ## here the component-total "H" is already converted to alkalinity (*-1)
    R.ALK   <- -1 * (- R.srs_NH3 + R.srs_H3PO4 + R.srs_HNO3 - 2 * R.srs_FeOH2
                     - 2 * R.srs_MnOH2 + 2 * R.srs_H2SO4 - 2 * R.srs_CaOH2)
    R.DIC    <- R.srs_CO2
    R.CH4    <- R.srs_CH4
    R.TOT_P  <- R.srs_H3PO4
    R.TOT_N5 <- R.srs_HNO3
    R.TOT_N3 <- R.srs_NH3
    R.TOT_S6 <- R.srs_H2SO4
    R.TOT_S2 <- R.srs_H2S
    R.O2     <- R.srs_O2
    R.N2     <- R.srs_N2
    R.Mn2    <- R.srs_MnOH2
    R.Fe2    <- R.srs_FeOH2
    R.Ca2    <- R.srs_CaOH2
    
    ## solids
    R.OM <- - R1 - R2 - R3 - R4 - R5 - R6
    R.OM_beta <- - R1b - R2b - R3b - R4b - R5b - R6b
    R.MnO2 <- (- 2 * R3 - 2 * R3b
              - R21
              - R23
              + R29)
    R.FeOH3 <- (- 4 * chi * R4 - 4 * chi * R4b
                - 2 * chi * R9
                + 4/4 * (1 - L_H3PO4) * p2s * R10
                + 2 * R21
                + R28)
    R.FeOH3_P <- (- 4 * (1 - chi) * R4 - 4 * (1 - chi) * R4b
                  - 2 * (1 - chi) * R9
                  + 4/4 * L_H3PO4 * p2s * R10)
    R.FeCO3 <- R7 - R8
    R.S0 <- R9 + R23
    R.FeS <- R13 - R14 - R24
    R.Viv <- R15 - R16
    R.CaCO3 <- R17 - R18
    R.Apatite <- R19 - R20
    R.MnCO3 <- R26 - R27

    # Irrigation (for solutes)
    I.ALK    <- k_iri * (ALK_up - ALK)
    I.DIC    <- k_iri * (DIC_up - DIC)
    I.CH4    <- k_iri * (CH4_up - CH4)
    I.TOT_P  <- k_iri * (H2PO4_up - TOT_P)
    I.TOT_N5 <- k_iri * (NO3_up - TOT_N5)
    I.TOT_N3 <- k_iri * (NH4_up - TOT_N3)
    I.N2     <- k_iri * (N2_up - N2)
    I.TOT_S6 <- k_iri * (SO4_up - TOT_S6)
    I.TOT_S2 <- k_iri * (H2S_up - TOT_S2)
    I.O2     <- k_iri * (O2_up - O2)
    I.Mn2    <- k_iri * (Mn2_up - Mn2)
    I.Fe2    <- k_iri * (Fe2_up - Fe2)
    I.Ca2    <- k_iri * (Ca2_up - Ca2)
    
    # total change in concentration of model species
    dCdt.ALK     <- R.ALK + tran.ALK$dC + I.ALK
    dCdt.DIC     <- R.DIC + tran.DIC$dC + I.DIC
    dCdt.CH4     <- R.CH4 + tran.CH4$dC + I.CH4
    dCdt.TOT_P   <- R.TOT_P + tran.TOT_P$dC + I.TOT_P
    dCdt.TOT_N5  <- R.TOT_N5 + tran.TOT_N5$dC + I.TOT_N5
    dCdt.TOT_N3  <- R.TOT_N3 + tran.TOT_N3$dC + I.TOT_N3 + tran.NH4_ads$dC * s2p
    dCdt.N2      <- R.N2  + tran.N2$dC + I.N2
    dCdt.TOT_S6  <- R.TOT_S6 + tran.TOT_S6$dC + I.TOT_S6
    dCdt.TOT_S2  <- R.TOT_S2 + tran.TOT_S2$dC + I.TOT_S2
    dCdt.O2      <- R.O2 + tran.O2$dC + I.O2
    dCdt.Mn2     <- R.Mn2 + tran.Mn2$dC + I.Mn2
    dCdt.Fe2     <- R.Fe2 + tran.Fe2$dC + I.Fe2
    dCdt.Ca2     <- R.Ca2 + tran.Ca2$dC + I.Ca2
    dCdt.OM      <- R.OM + tran.OM$dC
    dCdt.OM_beta <- R.OM_beta + tran.OM_beta$dC
    dCdt.MnO2    <- R.MnO2 + tran.MnO2$dC
    dCdt.FeOH3   <- R.FeOH3 + tran.FeOH3$dC
    dCdt.FeOH3_P <- R.FeOH3_P + tran.FeOH3_P$dC
    dCdt.FeCO3   <- R.FeCO3 + tran.FeCO3$dC
    dCdt.S0      <- R.S0 + tran.S0$dC
    dCdt.FeS     <- R.FeS + tran.FeS$dC
    dCdt.Viv     <- R.Viv + tran.Viv$dC
    dCdt.CaCO3   <- R.CaCO3 + tran.CaCO3$dC
    dCdt.Apatite <- R.Apatite + tran.Apatite$dC
    dCdt.MnCO3   <- R.MnCO3 + tran.MnCO3$dC
    
    # characteristic time scales
    # if dCdt.X = 0 -> timescale will be Inf
    time_scales <- c(
      ALK = ALK / dCdt.ALK,
      DIC = DIC / dCdt.DIC,
      CH4 = CH4 / dCdt.CH4,
      TOT_P = TOT_P / dCdt.TOT_P,
      TOT_N5 = TOT_N5 / dCdt.TOT_N5,
      TOT_N3 = TOT_N3 / dCdt.TOT_N3,
      N2 = N2 / dCdt.N2,
      TOT_S6 = TOT_S6 / dCdt.TOT_S6,
      TOT_S2 = TOT_S2 / dCdt.TOT_S2,
      O2 = O2 / dCdt.O2,
      Mn2 = Mn2 / dCdt.Mn2,
      Fe2 = Fe2 / dCdt.Fe2,
      Ca2 = Ca2 / dCdt.Ca2,
      OM = OM / dCdt.OM,
      OM_beta = OM_beta / dCdt.OM_beta,
      MnO2 = MnO2 / dCdt.MnO2,
      FeOH3 = FeOH3 / dCdt.FeOH3,
      FeOH3_P = FeOH3_P / dCdt.FeOH3_P,
      FeCO3 = FeCO3 / dCdt.FeCO3,
      S0 = S0 / dCdt.S0,
      FeS = FeS / dCdt.FeS,
      Viv = Viv / dCdt.Viv,
      CaCO3 = CaCO3 / dCdt.CaCO3,
      Apatite = Apatite / dCdt.Apatite,
      MnCO3 = MnCO3 / dCdt.MnCO3
    )
    # but we need to take the absolute value to find the correct minimum
    time_scales <- abs(time_scales)
    # set NaN values to a number (???)
    time_scales[is.na(time_scales)] <- 100
    # set 0 to a small number
    # time_scales[time_scales == 0] <- 1e-10

    return(
      list(
        c(
          dCdt.ALK = dCdt.ALK,
          dCdt.DIC = dCdt.DIC,
          dCdt.TOT_P = dCdt.TOT_P,
          dCdt.TOT_N5 = dCdt.TOT_N5,
          dCdt.TOT_N3 = dCdt.TOT_N3,
          dCdt.TOT_S6 = dCdt.TOT_S6,
          dCdt.TOT_S2 = dCdt.TOT_S2,
          dCdt.CH4 = dCdt.CH4,
          dCdt.O2 = dCdt.O2,
          dCdt.N2 = dCdt.N2,
          dCdt.Mn2 = dCdt.Mn2,
          dCdt.Fe2 = dCdt.Fe2,
          dCdt.Ca2 = dCdt.Ca2,
          dCdt.MnO2 = dCdt.MnO2,
          dCdt.OM = dCdt.OM,
          dCdt.OM_beta = dCdt.OM_beta,
          dCdt.FeOH3 = dCdt.FeOH3,
          dCdt.FeOH3_P = dCdt.FeOH3_P,
          dCdt.FeCO3 = dCdt.FeCO3,
          dCdt.S0 = dCdt.S0,
          dCdt.FeS = dCdt.FeS,
          dCdt.Viv = dCdt.Viv,
          dCdt.CaCO3 = dCdt.CaCO3,
          dCdt.Apatite = dCdt.Apatite,
          dCdt.MnCO3 = dCdt.MnCO3
        ),
        transport = list(
          tran.ALK = tran.ALK,
          tran.DIC = tran.DIC,
          tran.TOT_P = tran.TOT_P,
          tran.TOT_N5 = tran.TOT_N5,
          tran.TOT_N3 = tran.TOT_N3,
          tran.TOT_S6 = tran.TOT_S6,
          tran.TOT_S2 = tran.TOT_S2,
          tran.CH4 = tran.CH4,
          tran.O2 = tran.O2,
          tran.N2 = tran.N2,
          tran.Mn2 = tran.Mn2,
          tran.Fe2 = tran.Fe2,
          tran.Ca2 = tran.Ca2,
          tran.MnO2 = tran.MnO2,
          tran.OM = tran.OM,
          tran.OM_beta = tran.OM_beta,
          tran.FeOH3 = tran.FeOH3,
          tran.FeOH3_P = tran.FeOH3_P,
          tran.FeCO3 = tran.FeCO3,
          tran.S0 = tran.S0,
          tran.FeS = tran.FeS,
          tran.Viv = tran.Viv,
          tran.CaCO3 = tran.CaCO3,
          tran.Apatite = tran.Apatite,
          tran.MnCO3 = tran.MnCO3
        ),
        sumR = list(
          R.ALK     = R.ALK * grid.por$mid,
          R.DIC     = R.DIC * grid.por$mid,
          R.TOT_P   = R.TOT_P * grid.por$mid,
          R.TOT_N5  = R.TOT_N5 * grid.por$mid,
          R.TOT_N3  = R.TOT_N3 * grid.por$mid,
          R.TOT_S6  = R.TOT_S6 * grid.por$mid,
          R.TOT_S2  = R.TOT_S2 * grid.por$mid,
          R.CH4     = R.CH4 * grid.por$mid,
          R.O2      = R.O2 * grid.por$mid,
          R.N2      = R.N2 * grid.por$mid,
          R.Mn2     = R.Mn2 * grid.por$mid,
          R.Fe2     = R.Fe2 * grid.por$mid,
          R.Ca2     = R.Ca2 * grid.por$mid,
          R.MnO2    = R.MnO2 * grid.svf$mid,
          R.OM      = R.OM * grid.svf$mid,
          R.OM_beta = R.OM_beta * grid.svf$mid,
          R.FeOH3   = R.FeOH3 * grid.svf$mid,
          R.FeOH3_P = R.FeOH3_P * grid.svf$mid,
          R.FeCO3   = R.FeCO3 * grid.svf$mid,
          R.S0      = R.S0 * grid.svf$mid,
          R.FeS     = R.FeS * grid.svf$mid,
          R.Viv     = R.Viv * grid.svf$mid,
          R.CaCO3   = R.CaCO3 * grid.svf$mid,
          R.Apatite = R.Apatite * grid.svf$mid,
          R.MnCO3   = R.MnCO3 * grid.svf$mid
        ),
        irrigation = list(
          I.ALK     = I.ALK * grid.por$mid,
          I.DIC     = I.DIC * grid.por$mid,
          I.TOT_P   = I.TOT_P * grid.por$mid,
          I.TOT_N5  = I.TOT_N5 * grid.por$mid,
          I.TOT_N3  = I.TOT_N3 * grid.por$mid,
          I.TOT_S6  = I.TOT_S6 * grid.por$mid,
          I.TOT_S2  = I.TOT_S2 * grid.por$mid,
          I.CH4     = I.CH4 * grid.por$mid,
          I.O2      = I.O2 * grid.por$mid,
          I.N2      = I.N2 * grid.por$mid,
          I.Mn2     = I.Mn2 * grid.por$mid,
          I.Fe2     = I.Fe2 * grid.por$mid,
          I.Ca2     = I.Ca2 * grid.por$mid
        ),
        rates = list(
          R1  = R1  * grid.svf$mid,
          R2  = R2  * grid.svf$mid,
          R3  = R3  * grid.svf$mid,
          R4  = R4  * grid.svf$mid,
          R5  = R5  * grid.svf$mid,
          R6  = R6  * grid.svf$mid,
          R1b  = R1b  * grid.svf$mid,
          R2b  = R2b  * grid.svf$mid,
          R3b  = R3b  * grid.svf$mid,
          R4b  = R4b  * grid.svf$mid,
          R5b  = R5b  * grid.svf$mid,
          R6b  = R6b  * grid.svf$mid,
          R7  = R7  * grid.svf$mid,
          R8  = R8  * grid.svf$mid,
          R9  = R9  * grid.svf$mid,
          R10 = R10 * grid.svf$mid,
          R11 = R11 * grid.por$mid,
          R12 = R12 * grid.por$mid,
          R13 = R13 * grid.svf$mid,
          R14 = R14 * grid.svf$mid,
          R15 = R15 * grid.svf$mid,
          R16 = R16 * grid.svf$mid,
          R17 = R17 * grid.svf$mid,
          R18 = R18 * grid.svf$mid,
          R19 = R19 * grid.svf$mid,
          R20 = R20 * grid.svf$mid,
          R21 = R21 * grid.svf$mid,
          R22 = R22 * grid.por$mid, 
          R23 = R23 * grid.svf$mid,
          R24 = R24 * grid.svf$mid,
          R25 = R25 * grid.por$mid,
          R26 = R26 * grid.svf$mid,
          R27 = R27 * grid.svf$mid
        ),
        omega = list(
          siderite = omega_siderite,
          FeS = omega_FeS,
          Viv = omega_Viv,
          CaCO3 = omega_CaCO3,
          Apatite = omega_Apatite,
          MnCO3 = omega_MnCO3
        ),
        solute_equilibrium = solute_equilibrium,
        time_scales = time_scales
      )
    )
 })
}
```


## Solve Model

### 1) Steady State

Steady State is found in a three step procedure:

0) Solve steady state without precipitation reactions and without equilibrium solver.
1) Solve steady state without equilibrium solver. All reactions are controlled by component-totals. If necessary, pH dependent speciation is estimated for pH = 7.
2) Solve steady state with equilibrium solver. All reaction now are controlled by the actual concentrations.

```{r std0}
#| echo: true

initial <- rep(1e-4, N_grid * N_species)

if (run_std & run_std0) {
  std0 <- steady.1D(
    y = initial,
    func = model,
    parms = parameter, 
    dimens = N_grid,
    nspec = N_species,
    names = model_species,
    positive = TRUE,
    method = "stode",
    diff_coeffs = diffusion_coefficients,
    adv_vel = advective_velocities,
    k_iri = grid.k_iri$mid,
    solve_equilibrium = FALSE,
    precipitation = FALSE
  )

  save_and_upload_results(std0, "std0", "std")

} else {
  std0 <- readRDS("last_results/std0.rds")
}
```

```{r std1}
#| echo: true

if (run_std & run_std1) {
  std1 <- steady.1D(
    y = as.vector(std0$y),
    func = model,
    parms = parameter, 
    dimens = N_grid,
    nspec = N_species,
    names = model_species,
    positive = TRUE,
    method = "stode",
    diff_coeffs = diffusion_coefficients,
    adv_vel = advective_velocities,
    k_iri = grid.k_iri$mid,
    solve_equilibrium = FALSE,
    precipitation = TRUE
  )

  # add speciation afterwards
  # the first 7 model-species are tableau components
  # In the model we have alkalinity (column 1) - but here we need TOT_H = -ALK
  component_total <- c(-1*std1$y[,1], std1$y[,2:7])
  ## only DIC and H2S considered in Wang and Van Capellen
  #component_total <- c(-1*std1$y[,1], std1$y[,2], rep(0, N_grid * 4), std1$y[,7])
  std1[["solute_equilibrium"]] <- solve_tableau(component_total, tableau, logK, N_grid, tableau_species)

  save_and_upload_results(std1, "std1", "std")

} else {
  std1 <- readRDS("last_results/std1.rds")
}
```

```{r std2}
#| echo: true

if (run_std & run_std2) {
  std2 <- steady.1D(
    y = as.vector(std1$y),
    func = model,
    parms = parameter,
    dimens = N_grid,
    nspec = N_species,
    names = model_species,
    positive = TRUE,
    method = "stode", # stode gives back result, even if steady state is not reached
                      # stodes does not return intermediate result
    maxiter = 100,
    diff_coeffs = diffusion_coefficients,
    adv_vel = advective_velocities,
    k_iri = grid.k_iri$mid,
    solve_equilibrium = TRUE,
    precipitation = TRUE
  )

  save_and_upload_results(std2, "std2", "std")

} else {
  std2 <- readRDS("last_results/std2.rds")
}
```


#### Profiles: Component-Totals

- std1: gray
- std2: black

```{r plot std2}
#| column: screen-inset
#| layout-ncol: 5
#| fig.height: 3.5
#| fig.width: 3.5

plot_std_profiles(std1, std2, grid)
```

#### Profiles: Speciation

::: {.panel-tabset}

##### std1

```{r plot std1 speciation}
#| layout-ncol: 2
#| fig.height: 3.7
#| fig.width: 5

speciation_std1 <- std1$solute_equilibrium$species_conc
colnames(speciation_std1) <- tableau_species

plot_speciation(speciation_std1, grid)
```

##### std2

```{r plot std2 speciation}
#| layout-ncol: 2
#| fig.height: 3.7
#| fig.width: 5

speciation_std2 <- std2$solute_equilibrium$species_conc
colnames(speciation_std2) <- tableau_species

plot_speciation(speciation_std2, grid)
```

:::

#### Profile: pH

```{r plot pH}
#| out.width: 100%

pH_std1 <- -log10(speciation_std1[,1] * 10^-3) # mol/m3 * 10^-3 m3/L
pH_std2 <- -log10(speciation_std2[,1] * 10^-3) # mol/m3 * 10^-3 m3/L

plot_simple_comparision(pH_std1, pH_std2, "std1", "std2", grid, "pH", "depth (m)")
```

#### Profiles: Saturation

```{r plot saturation}
#| layout-ncol: 2
#| fig.height: 4.2
#| fig.width: 5

plot_simple_comparision(
  a = std1$omega$siderite,
  b = std2$omega$siderite,
  a_name = "std1",
  b_name = "std2",
  grid = grid,
  xlab = "omega siderite",
  ylab = "depth (m)",
  log = "x"
)
abline(v = 1, lwd = 2, col = "red")

plot_simple_comparision(
  a = std1$omega$FeS,
  b = std2$omega$FeS,
  a_name = "std1",
  b_name = "std2",
  grid = grid,
  xlab = "omega FeS",
  ylab = "depth (m)",
  log = "x"
)
abline(v = 1, lwd = 2, col = "red")

plot_simple_comparision(
  a = std1$omega$Viv,
  b = std2$omega$Viv,
  a_name = "std1",
  b_name = "std2",
  grid = grid,
  xlab = "omega Viv",
  ylab = "depth (m)",
  log = "x"
)
abline(v = 1, lwd = 2, col = "red")

plot_simple_comparision(
  a = std1$omega$CaCO3,
  b = std2$omega$CaCO3,
  a_name = "std1",
  b_name = "std2",
  grid = grid,
  xlab = "omega CaCO3",
  ylab = "depth (m)",
  log = "x"
)
abline(v = 1, lwd = 2, col = "red")

plot_simple_comparision(
  a = std1$omega$Apatite,
  b = std2$omega$Apatite,
  a_name = "std1",
  b_name = "std2",
  grid = grid,
  xlab = "omega Apatite",
  ylab = "depth (m)",
  log = "x"
)
abline(v = 1, lwd = 2, col = "red")

plot_simple_comparision(
  a = std1$omega$MnCO3,
  b = std2$omega$MnCO3,
  a_name = "std1",
  b_name = "std2",
  grid = grid,
  xlab = "omega MnCO3",
  ylab = "depth (m)",
  log = "x"
)
abline(v = 1, lwd = 2, col = "red")
```

#### Profiles: Reaction Rates

##### OM Degradation

:::: {.columns}

::: {.column}
**std1**
```{r plot rates OM Degradation std1}
#| out.width: 100%

plot_std_rates(std1, c("R1", "R2", "R3", "R4", "R5", "R6"))
```
:::

::: {.column}
**std2**
```{r plot rates OM Degradation std2}
#| out.width: 100%

plot_std_rates(std2, c("R1", "R2", "R3", "R4", "R5", "R6"))
```
:::

::::

\

:::: {.columns}

::: {.column}
**std1**
```{r plot rates OM_beta Degradation std1}
#| out.width: 100%

plot_std_rates(std1, c("R1b", "R2b", "R3b", "R4b", "R5b", "R6b"))
```
:::

::: {.column}
**std2**
```{r plot rates OM_beta Degradation std2}
#| out.width: 100%

plot_std_rates(std2, c("R1b", "R2b", "R3b", "R4b", "R5b", "R6b"))
```
:::

::::

\

##### H2S Oxidation

:::: {.columns}

::: {.column}
**std1**
```{r plot rates H2S Oxidation std1}
#| out.width: 100%

plot_std_rates(std1, c("R11"))
```
:::

::: {.column}
**std2**
```{r plot rates H2S Oxidation std2}
#| out.width: 100%

plot_std_rates(std2, c("R11"))
```
:::

::::

\

##### SO4-Reduction Coupled to AOM

:::: {.columns}

::: {.column}
**std1**
```{r plot rates SO4-Reduction std1}
#| out.width: 100%

plot_std_rates(std1, c("R12"))
```
:::

::: {.column}
**std2**
```{r plot rates SO4-Reduction std2}
#| out.width: 100%

plot_std_rates(std2, c("R12"))
```
:::

::::

\

##### Siderite Precipitation

:::: {.columns}

::: {.column}
**std1**
```{r plot rates Siderite Precip std1}
#| out.width: 100%

plot_std_rates(std1, c("R7", "R8"))
```
:::

::: {.column}
**std2**
```{r plot rates Siderite Precip std2}
#| out.width: 100%

plot_std_rates(std2, c("R7", "R8"))
```
:::

::::

\

##### FeOH3 / FeOH3~H3PO4-reduction coupled to sulphide oxidation

:::: {.columns}

::: {.column}
**std1**
```{r plot rates FeOH3 reduction std1}
#| out.width: 100%

plot_std_rates(std1, c("R9"))
```
:::

::: {.column}
**std2**
```{r plot rates FeOH3 reduction std2}
#| out.width: 100%

plot_std_rates(std2, c("R9"))
```
:::

::::

\

##### FeOH3 / FeOH3~H3PO4-formation

:::: {.columns}

::: {.column}
**std1**
```{r plot rates FeOH3 formation std1}
#| out.width: 100%

plot_std_rates(std1, c("R10"))
```
:::

::: {.column}
**std2**
```{r plot rates FeOH3 formation std2}
#| out.width: 100%

plot_std_rates(std2, c("R10"))
```
:::

::::

\

##### FeS Precipitation / Dissolution

:::: {.columns}

::: {.column}
**std1**
```{r plot rates Fes precip/diss std1}
#| out.width: 100%

plot_std_rates(std1, c("R13", "R14"))
```
:::

::: {.column}
**std2**
```{r plot rates FeS precip/diss std2}
#| out.width: 100%

plot_std_rates(std2, c("R13", "R14"))
```
:::

::::

\

##### Vivianite Precipitation / Dissolution

:::: {.columns}

::: {.column}
**std1**
```{r plot rates Viv precip/diss std1}
#| out.width: 100%

plot_std_rates(std1, c("R15", "R16"))
```
:::

::: {.column}
**std2**
```{r plot rates Viv precip/diss std2}
#| out.width: 100%

plot_std_rates(std2, c("R15", "R16"))
```
:::

::::

\

##### CaCO3 Precipitation / Dissolution

:::: {.columns}

::: {.column}
**std1**
```{r plot rates CaCO3 precip/diss std1}
#| out.width: 100%

plot_std_rates(std1, c("R17", "R18"))
```
:::

::: {.column}
**std2**
```{r plot rates CaCO3 precip/diss std2}
#| out.width: 100%

plot_std_rates(std2, c("R17", "R18"))
```
:::

::::

\

##### Apatite Precipitation / Dissolution

:::: {.columns}

::: {.column}
**std1**
```{r plot rates Apatite precip/diss std1}
#| out.width: 100%

plot_std_rates(std1, c("R19", "R20"))
```
:::

::: {.column}
**std2**
```{r plot rates Apatite precip/diss std2}
#| out.width: 100%

plot_std_rates(std2, c("R19", "R20"))
```
:::

::::


#### Rate Comparision

- left: std1
- right: std2

```{r plot rate comparision}
#| layout-ncol: 2

plot_std_rate_comparision("ALK", std1, grid)
plot_std_rate_comparision("ALK", std2, grid)

plot_std_rate_comparision("DIC", std1, grid)
plot_std_rate_comparision("DIC", std2, grid)

plot_std_rate_comparision("TOT_P", std1, grid)
plot_std_rate_comparision("TOT_P", std2, grid)

plot_std_rate_comparision("TOT_N5", std1, grid)
plot_std_rate_comparision("TOT_N5", std2, grid)

plot_std_rate_comparision("TOT_N3", std1, grid)
plot_std_rate_comparision("TOT_N3", std2, grid)

plot_std_rate_comparision("TOT_S6", std1, grid)
plot_std_rate_comparision("TOT_S6", std2, grid)

plot_std_rate_comparision("TOT_S2", std1, grid)
plot_std_rate_comparision("TOT_S2", std2, grid)

plot_std_rate_comparision("CH4", std1, grid)
plot_std_rate_comparision("CH4", std2, grid)

plot_std_rate_comparision("O2", std1, grid)
plot_std_rate_comparision("O2", std2, grid)

plot_std_rate_comparision("N2", std1, grid)
plot_std_rate_comparision("N2", std2, grid)

plot_std_rate_comparision("Mn2", std1, grid)
plot_std_rate_comparision("Mn2", std2, grid)

plot_std_rate_comparision("Fe2", std1, grid)
plot_std_rate_comparision("Fe2", std2, grid)

plot_std_rate_comparision("Ca2", std1, grid)
plot_std_rate_comparision("Ca2", std2, grid)
```


### 2) Dynamic Scenario

The dynamic scenario is initialized with the results of std2.
For now the only dynamic parameters is the flux of $\ce{Fe(OH)3}$ at the top.

In February of the first year 100g (~1.8 mol) are added. The total added mass is calculated as
```{r FeOH3_up eq.1}
#| echo: true
#| eval: false

added mass [mol/m²] = flux [mol/(m²*yr)] * timespan [yr]
```

Therefore the specified flux in February is
```{r FeOH3_up eq.2}
#| echo: true
#| eval: false

flux [mol/(m²*yr)] = 1.8 [mol/m²] / (1/12) [yr] = 21.6 [mol/(m²*yr)]
```

```{r trans}
#| echo: true

timesteps <- seq(from = 0, to = 2, by = 1/12)

if (run_trans) {
  system.time(trans <- ode.1D(
    y = as.vector(std2$y), 
    time = timesteps, 
    func = model, 
    parms = parameter,
    dimens = N_grid,
    nspec = N_species,
    names = model_species,
    #method = "lsoda", 
    #verbose = TRUE, 
    diff_coeffs = diffusion_coefficients,
    adv_vel = advective_velocities,
    solve_equilibrium = TRUE,
    precipitation = TRUE
  ))

  trans <- split_trans_results(trans, grid, tableau_species)
  save_and_upload_results(trans, "trans", "trans")

} else {
  trans <- readRDS("last_results/trans.rds")
}
```

```{r plot trans conc contour}

#contour_plots <- list()

#for (species in model_species) {
#  contour_plots[[species]] <- plot_trans_conc_contour(trans_split, species)
#}

plot_trans_conc_contour(trans, "FeOH3")

plot_trans_conc_contour(trans, "Viv")

plot_trans_conc_lines(trans, "FeOH3", c(0/12, 3/12, 6/12, 9/12, 1))

#plot(
#  x = subset(trans_split$species, name == "FeOH3" & time == 0)$value,
#  y = subset(trans_split$species, name == "FeOH3" & time == 0)$depth,
#  type = "l",
#  ylim = c(0.1, 0)
#)
```


## Mass Balances

### Species-based Mass Balances

The change in concentration of a species $i$, integrated over depth and in relation to the area $A$ is

$$\frac{dC_{A,i}}{dt} \; \left( \frac{mol}{m^2 \cdot a} \right) = J(a)_i - J(b)_i + \int_{a}^b \Psi \sum R_i$$

with

- $J(a)_i$: flux of species $i$ across upper boundary
- $J(b)_i$: flux of species $i$ across lower boundary
- $\Psi$: volume fraction.

```{r std species-based mass balances}
#| echo: true
#| code-fold: true

species_mass_balance <- function(species_vector, std, grid) {
  
  species_mass_balance_inner <- function(species, std, grid) {
  
    tran.species <- paste0("tran.", species)
    flux_up   <- std$transport[[tran.species]]$flux.up
    flux_down <- std$transport[[tran.species]]$flux.down
    netto_flux <- flux_up - flux_down
    
    R.species <- paste0("R.", species)
    sumR_integrated <- sum(std$sumR[[R.species]] * grid$dx)
    
    bilanz <- netto_flux + sumR_integrated
    
    return(data.frame(
      species = species,
      flux_up = flux_up,
      flux_down = flux_down,
      netto_flux = netto_flux,
      sumR_integrated = sumR_integrated,
      bilanz = bilanz
    ))
  }
  
  species_based_mass_balance <- data.frame(
  "Species" = NULL,
  "Flux up" = NULL,
  "Flux down" = NULL,
  "Netto Flux" = NULL,
  "Integral Sum R" = NULL,
  "Bilanz" = NULL
  )
  
  for (species in species_vector) {
    species_based_mass_balance <- rbind(
      species_based_mass_balance,
      species_mass_balance_inner(species, std, grid)
    )
  }
  
  return(species_based_mass_balance)
}
```

```{r print species based mass balances}
species_mass_balance_std1 <- species_mass_balance(model_species, std1, grid)
#species_mass_balance_std2 <- species_mass_balance(model_species, std2, grid)

knitr::kable(
  species_mass_balance_std1,
  caption = "std1 (mol m-2 yr-1)",
  digits = 25,
  format.args = list(format = "e", digits = 4)
)
#knitr::kable(
#  species_mass_balance_std2,
#  caption = "std2 (mol m-2 yr-1)",
#  digits = 25,
#  format.args = list(format = "e", digits = 4)
#)
```


### Element-based Mass Balances

The total amount of an chemical element $e$ is not influenced by chemical reactions.
Therefore, the over the depth integrated and summed up reaction rates of of species $i$ that contain a chemical element is 0.

$$\sum \left[ \frac{e}{i} \cdot \int_{a}^b \Psi \sum R_i \right]_e = 0$$

From this follows, that under stationary conditions the flux in and out of the sediment domain cancel out.

$$\frac{dC_{A,e}}{dt} \; \left( \frac{mol}{m^2 \cdot a} \right) = J(a)_e - J(b)_e = 0$$

```{r element based mass balance}
#| echo: true
#| code-fold: true

element_mass_balance <- function(elemental_composition, std, grid) {
  
  species_sumR_integrated <- function(species, std, grid) {
    
    R.species <- paste0("R.", species)
    sumR_integrated <- sum(std$sumR[[R.species]] * grid$dx)
    
    return(sumR_integrated)
  }

  species_boundary_fluxes <- function(species, std, grid) {
  
    tran.species <- paste0("tran.", species)
    flux_up   <- std$transport[[tran.species]]$flux.up
    flux_down <- std$transport[[tran.species]]$flux.down
    
    return(list(
      flux_up = flux_up,
      flux_down = flux_down
    ))
  }

  elements <- names(elemental_composition)
  
  overview <- data.frame(
    "Element" = NULL,
    "Netto Flux" = NULL,
    "Integral Sum R" = NULL
  )

  detailed <- list()
  
  for (element in elements) {

    composition <- elemental_composition[[element]]

    detailed_part <- data.frame(
      "Species" = NULL,
      "Flux up" = NULL,
      "Flux down" = NULL,
      "Intergal Sum R" = NULL
    )

    # for every species in which an element is present ...
    for (i in seq_len(nrow(composition))) {
      species <- composition$species[i]
      stoic   <- composition$stoic[i]

      sumR <- species_sumR_integrated(species, std, grid) * stoic

      boundary_fluxes <- species_boundary_fluxes(species, std, grid)
      flux_up <- boundary_fluxes$flux_up * stoic
      flux_down <- boundary_fluxes$flux_down * stoic

      detailed_part <- rbind(
        detailed_part,
        data.frame(
          "Species" = species,
          "Flux up" = flux_up,
          "Flux down" = flux_down,
          "Integral Sum R" = sumR
        )
      )
    }

    detailed[[element]] <- detailed_part

    net_flux <- sum(detailed_part[["Flux.up"]]) - sum(detailed_part[["Flux.down"]])
    net_rate <- sum(detailed_part[["Integral.Sum.R"]])

    overview <- rbind(
      overview,
      data.frame(
        "Element" = element,
        "Netto Flux" = net_flux,
        "Integral Sum R" = net_rate
      )
    )
  }
  
  return(list(
    overview = overview,
    detailed = detailed
  ))
}
```

#### Elemental Composition of Model-Species
```{r elemental composition}

elemental_composition <- list(
  C = data.frame(
    species = c("DIC", "CH4", "OM", "FeCO3", "CaCO3", "OM_beta"),
    stoic   = c(1, 1, 1, 1, 1, 1)
  ),
  N = data.frame(
    species = c("TOT_N5", "TOT_N3", "N2", "OM", "OM_beta"),
    stoic   = c(1, 1, 2, reaction_parameter[["OM_b"]], reaction_parameter[["OM_b"]])
  ),
  P = data.frame(
    species = c("TOT_P", "OM", "FeOH3_P", "Viv", "Apatite", "OM_beta"),
    stoic   = c(1, reaction_parameter[["OM_c"]], reaction_parameter[["lambda"]], 2, 2, reaction_parameter[["OM_c"]])
  ),
  S = data.frame(
    species = c("TOT_S6", "TOT_S2", "S0", "FeS"),
    stoic   = c(1, 1, 1, 1)
  ),
  Fe = data.frame(
    species = c("Fe2", "FeOH3", "FeOH3_P", "FeCO3", "FeS", "Viv"),
    stoic   = c(1, 1, 1, 1, 1, 3)
  ),
  Mn = data.frame(
    species = c("Mn2", "MnO2"),
    stoic   = c(1, 1)
  ),
  Ca = data.frame(
    species = c("Ca2", "CaCO3", "Apatite"),
    stoic   = c(1, 1, 3)
  )
)
```

```{r print elemental composition}
#| layout-ncol: 4
#| results: "asis"

for (i in seq_len(length(elemental_composition))) {
  print(knitr::kable(
    elemental_composition[[i]],
    caption = names(elemental_composition[i]),
    digits = 5,
    valign = "t",
    booktabs = TRUE
  ))
}
```

#### Overview Mass Balances
```{r print element based mass balances: overview}
#| layout-ncol: 2

element_mass_balance_std1 <- element_mass_balance(elemental_composition, std1, grid)
element_mass_balance_std2 <- element_mass_balance(elemental_composition, std2, grid)

knitr::kable(
  element_mass_balance_std1$overview,
  caption = "std1 (mol m-2 yr-1)",
  digits = 20,
  format.args = list(format = "e", digits = 2),
  valign = "t"
)
knitr::kable(
  element_mass_balance_std2$overview,
  caption = "std2 (mol m-2 yr-1)",
  digits = 20,
  format.args = list(format = "e", digits = 2),
  valign = "t"
)
```

#### Detailed Mass Balances

##### std1
```{r print element based mass balance: detailed std1}
#| layout-ncol: 3
#| results: "asis"

for (i in seq_len(length(element_mass_balance_std1$detailed))) {
  print(knitr::kable(
    element_mass_balance_std1$detailed[[i]],
    caption = paste("std1:", names(element_mass_balance_std1$detailed)[i], "(mol m-2 yr-1)"),
    digits = 5,
    valign = "t",
    booktabs = TRUE
  ))
}
```

##### std2
```{r print element based mass balance: detailed std2}
#| layout-ncol: 3
#| results: "asis"

for (i in seq_len(length(element_mass_balance_std2$detailed))) {
  print(knitr::kable(
    element_mass_balance_std2$detailed[[i]],
    caption = paste("std2:", names(element_mass_balance_std2$detailed)[i], "(mol m-2 yr-1)"),
    digits = 5,
    valign = "t",
    booktabs = TRUE
  ))
}
```

### Depth Integrated Reaction Rates

```{r depth integrated reaction rates}
#| echo: true
#| code-fold: true

depth_integrated_reactionrates <- function(std, grid) {
  
  integrated_reactionrate <- function(rate, std, grid) {
    
    rate_integrated <- sum(std$rates[[rate]] * grid$dx)
    
    return(rate_integrated)
  }

  rates <- names(std$rates)
  
  result <- data.frame(
    "Name" = NULL,
    "Rate" = NULL
  )
  
  for (rate in rates) {
  
    result <- rbind(
      result,
      data.frame(
        "Name" = rate,
        "Rate" = integrated_reactionrate(rate, std, grid)
      )
    )
  }
  
  return(result)
}
```

```{r print depth integrated rates}
#| layout-ncol: 2
#| results: "asis"

depth_int_rates_std1 <- depth_integrated_reactionrates(std1, grid)
depth_int_rates_std2 <- depth_integrated_reactionrates(std2, grid)

knitr::kable(
  depth_int_rates_std1,
  caption = paste("std1: depth integrated reaction rates (mol m-2 yr-1)"),
  digits = 5,
  valign = "t",
  booktabs = TRUE
)

knitr::kable(
  depth_int_rates_std2,
  caption = paste("std2: depth integrated reaction rates (mol m-2 yr-1)"),
  digits = 5,
  valign = "t",
  booktabs = TRUE
)
```

